...
function add_tender_comment() {
    ...
    data.proposalid = proposal_id;
    $('body').append('<div class="dt-loader"></div>');
    $.post(admin_url + 'tenders/add_proposal_comment', data).done(function(response) {
        response = JSON.parse(response);
        $('body').find('.dt-loader').remove();
        if (response.success == true) {
            $('#comment').val('');
...
function get_inquiry_comments() {
    if (typeof(proposal_id) == 'undefined') { return; }
    requestGet('inquiries/get_proposal_comments/' + proposal_id).done(function(response) {
        $('body').find('#proposal-comments').html(response);
    });
}

function get_tender_comments() {
    if (typeof(proposal_id) == 'undefined') { return; }
    requestGet('tenders/get_proposal_comments/' + proposal_id).done(function(response) {
        $('body').find('#proposal-comments').html(response);
    });
}
