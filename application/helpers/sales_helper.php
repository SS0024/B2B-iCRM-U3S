...
function get_stock_transaction_items_by_type($type, $id)
{
    $CI = &get_instance();
    $CI->db->select();
    $CI->db->from('tblstock_transactions');
    $CI->db->where('rel_id', $id);
    $CI->db->where('rel_type', $type);
...
{
    $CI = &get_instance();
    $CI->db->select('discount_percent, discount_type, discount_total, subtotal, packing_and_forwarding');
    $CI->db->from($table);
    $CI->db->where('id', $id);
        ...
        if (count($item_taxes) > 0) {
            foreach ($item_taxes as $tax) {
                ...
                if ($tax_not_calc == true) {
                    ...
                    if (isset($data->packing_and_forwarding) && $data->packing_and_forwarding > 0) {
                        $new_tax_amount = (($data->packing_and_forwarding / $data->subtotal) * $item['rate']) + $item['rate'];
                        $new_tax = $new_tax_amount * $tax['taxrate'] / 100;
                        array_push($taxes[$tax['taxname']]['total'], $new_tax);
                    } else {
...
function add_new_sales_item_post($item, $rel_id, $rel_type)
{
    ...
    $CI->db->insert('tblitems_in', [
        ...
        'qty' => $item['qty'],
        'rate' => number_format((isset($item['rate']) ? $item['rate'] : (isset($item['item_amount']) ? $item['item_amount'] : 0)), get_decimal_places(), '.', ''),
        'rel_id' => $rel_id,
        'fab_no' => isset($item['fab_no']) ? $item['fab_no'] : '',
        'group_id' => isset($item['group_id']) ? $item['group_id'] : '',
        'rel_type' => $rel_type,
        'item_order' => $item['order'],
        'unit' => $item['unit'],
        'item_amount' => isset($item['item_amount']) ? number_format($item['item_amount'], get_decimal_places(), '.', '') : 0.0,
        'item_discount' => isset($item['item_discount']) ? number_format($item['item_discount'], get_decimal_places(), '.', '') : 0,
        'item_other_status' => isset($item['item_other_status']) ? $item['item_other_status'] : '',
        'hold_item' => isset($item['hold_item']) ? $item['hold_item'] : 0,
        'item_remark' => isset($item['item_remark']) ? $item['item_remark'] : ''
    ]);
...
function update_sales_item_post($item_id, $data, $field = '')
{
    ...
    //     } elseif ($field == 'item_amount') {
    //         $update[$field] = $data['item_amount'];
    ...
    $update = [
        ...
        'rate' => number_format(isset($data['rate']) ? $data['rate'] : (isset($data['item_amount']) ? $data['item_amount'] : 0), get_decimal_places(), '.', ''),
        'qty' => $data['qty'],
        'unit' => $data['unit'],
        'item_amount' => $data['item_amount'],
        'item_discount' => isset($data['item_discount']) ? number_format($data['item_discount'], get_decimal_places(), '.', '') : 0,
        'item_other_status' => isset($data['item_other_status']) ? $data['item_other_status'] : '',
        'hold_item' => isset($data['hold_item']) ? $data['hold_item'] : 0,
        'item_remark' => isset($data['item_remark']) ? $data['item_remark'] : ''
    ];
    // }
...
if (!function_exists('get_table_items_and_taxes')) {
     ...
     * @return array
     */
    function get_table_items_and_taxes($items, $type, $admin_preview = false, $is_bulk = 0,$isDiscountDisplay = 1)
    {
        ...
        $result['itm_data'] = $items;
        foreach ($items as $item) {
            $CI = &get_instance();
            $CI->db->select('*');
            $CI->db->from('tblitems');
            $CI->db->where('description', $item['description']);
            $recdata = $CI->db->get()->row();

            $CI->db->select('*');
            $CI->db->from('tblitems_groups');
            $CI->db->where('id', $item['group_id']);
            $groupdata = $CI->db->get()->row();

            if (isset($recdata->brand)){
                $CI->db->select('*');
                $CI->db->from('tblitems_brands');
                $CI->db->where('id', $recdata->brand);
                $branddata = $CI->db->get()->row();
            }
            ...
            $tdFirstSortable = '';
            $statusColor = '';
            if ($item['item_other_status'] == 'lost' || $item['item_other_status'] == 'lostConvert') {
                $statusColor = 'border-left: 4px solid #fc2d42';
            } elseif ($item['item_other_status'] == 'won' || $item['item_other_status'] == 'wonConvert') {
                $statusColor = 'border-left: 4px solid #84c529;';
            } elseif ($item['item_other_status'] == 'partial_hold') {
            ...
            if (class_exists('pdf')) {
                ...
                $trAttributes .= ' style="' . $statusColor . '"';
            }
            ...
            if (isset($groupdata->name) && !empty($groupdata->name)) {
                $itemHTML .= '<br><span style="color:#424242;">Group: ' . $groupdata->name . '</span>';
            }
            if (isset($branddata->name) && !empty($branddata->name)) {
                $itemHTML .= '<br><span style="color:#424242;">Brand: ' . $branddata->name . '</span>';
            }
            $itemHTML .= '</td>';
            ...
            if (!class_exists('pdf') && $type == 'purchaseorder'){
                $itemHTML .= '<td align="right">' . floatVal($item['hold_item']);
                if ($item['unit']) {
                    $itemHTML .= ' ' . $item['unit'];
                }

                $itemHTML .= '</td>';
                $itemHTML .= '<td align="right">' . floatVal($item['converted_qty']);
                if ($item['unit']) {
                    $itemHTML .= ' ' . $item['unit'];
                }
            ...
            if ($type != 'requsite_form'){
                if ($is_bulk == 0) {
                    $itemHTML .= '<td align="right">' . _format_number($item['item_amount']) . '</td>';
                }
                ...
                if (count($item_taxes) > 0) {
                    ...
                    foreach ($item_taxes as $tax) {
                        $calc_tax = 0;
                        $tax_not_calc = false;
                        $ammnt = $item['qty'] * $item['rate'];
                        $ttl_tx = ($tax['taxrate'] / 100) * $ammnt;

                        /* $tax_qty[$tax['taxnum']['total_qtys']] +=  $item['qty'];
                        $tax_qty[$tax['taxnum']['total_prcs']] +=  $ammnt;
                        $tax_qty[$tax['taxnum']['total_tprs']] +=  $ttl_tx; */
                        if(!isset($tax_qty[$tax['taxnum']]['total_qtys'])){
                            $tax_qty[$tax['taxnum']]['total_qtys'] = 0;
                        }
                        if(!isset($tax_qty[$tax['taxnum']]['total_prcs'])){
                            $tax_qty[$tax['taxnum']]['total_prcs'] = 0;
                        }
                        if(!isset($tax_qty[$tax['taxnum']]['total_tprs'])){
                            $tax_qty[$tax['taxnum']]['total_tprs'] = 0;
                        }
                        $tax_qty[$tax['taxnum']]['tax_name'] = $tax['taxname'];
                        $tax_qty[$tax['taxnum']]['total_qtys'] += $item['qty'];
                        $tax_qty[$tax['taxnum']]['total_prcs'] += $ammnt;
                        $tax_qty[$tax['taxnum']]['total_tprs'] += $ttl_tx;
                ...
                if ($type != '' && in_array($type, array('invoice', 'estimate', 'proposal', 'purchaseorder', 'delivery_challan', 'inquiry'))) {
                    $item_discounted_amount_with_quantity = $hook_data['amount'];
                    $item_amount_with_quantity = $item['qty'] * $item['item_amount'];
                    if ($is_bulk == 0) {
                        if (isset($item['item_discount']) && $item['item_discount'] > 0) {
                            $itemHTML .= '<td align="right">(' . _format_number($item['item_discount']) . '%) <br/> -' . _format_number(($item_amount_with_quantity - $item_discounted_amount_with_quantity)) . '</td>';
                        }/*else{
    					$itemHTML .= '<td align="right">(' . _format_number($item['item_discount']) . '%)</td>';
    				}*/


                        if (isset($item['item_discount']) && $item['item_discount'] > 0) {
                            //$discountCnt = 1;
                            $itemHTML .= '<td class="amount" align="right">' . _format_number($item_discounted_amount_with_quantity) . '<br/><del>' . _format_number($item_amount_with_quantity) . '</del></td>';
                        } else {
                            if($isDiscountDisplay){
                                $itemHTML .= '<td class="amount" align="right">0.00</td>';
                            }
                            $itemHTML .= '<td class="amount" align="right">' . _format_number($item_discounted_amount_with_quantity) . '</td>';
                        }
                    }
                } else {
        ...
        //$result['is_disc'] = $discountCnt;
        $result['tax_qty'] = $tax_qty;
...
if (!function_exists('get_table_items_and_taxes_new')) {
    /**
     * Function for all table items HTML and PDF
     * @param array $items all items
     * @param  [type]  $type          where do items come form, eq invoice,estimate,proposal etc..
     ...
     * @return array
     */
    function get_table_items_and_taxes_new($items, $type, $admin_preview = false)
    {
        $cf = count($items) > 0 ? get_items_custom_fields_for_table_html($items[0]['rel_id'], $type) : [];
...
if (!function_exists('get_table_items')) {
    /**
     * Function for all table items HTML and PDF
     * @param array $items all items
     * @param  [type]  $type          where do items come form, eq invoice,estimate,proposal etc..
     ...
     * @return array
     */
    function get_table_items($items, $type, $admin_preview = false)
    {
        $cf = count($items) > 0 ? get_items_custom_fields_for_table_html($items[0]['rel_id'], $type) : [];
...
if (!function_exists('get_stock_table_items')) {
    /**
     * Function for all table items HTML and PDF
     * @param array $items all items
     * @param  [type]  $type          where do items come form, eq invoice,estimate,proposal etc..
     ...
     * @return array
     */
    function get_stock_table_items($items, $type, $admin_preview = false)
    {
        ...
        foreach ($items as $item) {
            ...
            if ($type == 'stock_in') {
                $itemHTML .= '<tr width="100%" ' . $trAttributes . '>';
                $itemHTML .= '<td' . $tdFirstSortable . ' width="5%" align="center">' . $i . '</td>';

                $itemHTML .= '<td width="80%" class="description" align="left;">';

                if (!empty($item['product_code'])) {
                    $itemHTML .= '<span style="font-size:' . (isset($font_size) ? $font_size + 4 : '') . 'px;"><strong>' . $item['product_code'] . '</strong></span>';
                ...
                if (!empty($item['product_name'])) {
                    $itemHTML .= '<span style="color:#424242;">' . $item['product_name'] . '</span>';
                }
                $stockTransaction = get_stock_transaction_items_by_type('purchase', $item['id']);
                $itemHTML .= '<br>';
                foreach ($stockTransaction as $stockDetails) {
                    $itemHTML .= '<br> <b>Stock transaction on:</b> ' . _dt($stockDetails['added_on']);
                    $itemHTML .= ', <b>Stock amount:</b> ' . $stockDetails['quantity'];
                }
                $itemHTML .= '</td>';
                $itemHTML .= '<td align="right"  width="15%">' . floatVal($item['quantity_balance']);
                ...
                $stockTransaction = get_stock_transaction_items_by_type(($type =='stock_out_extra' ? 'sale_extra' :'sale'), $item['id']);
                $itemHTML .= '<br>';
                foreach ($stockTransaction as $stockDetails) {
                    $itemHTML .= '<br> <b>Stock transaction on:</b> ' . _dt($stockDetails['added_on']);
                    $itemHTML .= ', <b>Stock amount:</b> ' . $stockDetails['quantity'];
                ...
                $itemHTML .= '</td>';
                $itemHTML .= '<td align="right"  width="15%">' . floatVal($item['qty']);
...
function get_div_cons_by_type($type, $id = null, $where =[], $is_distinct = false)
{
    /*SELECT  FROM `tblitems_in_div_con` INNER JOIN `tblcontacts` ON `tblitems_in_div_con`.`contact_id` = `tblcontacts`.`id` INNER JOIN `tbldivision` ON `tblitems_in_div_con`.`division_id` = `tbldivision`.`id` WHERE 1*/
    $CI = &get_instance();
    if ($is_distinct){
        $CI->db->select('DISTINCT CONCAT(`tblcontacts`.`firstname`, " ", `tblcontacts`.`lastname`," (", `tblcontacts`.`title`,")"," (", `tblcontacts`.`phonenumber`,")") as `contact_name`,`tblitems_in_div_con`.`contact_id` AS `con`');
    }else{
        $CI->db->select('`tblitems_in_div_con`.`rel_id` AS `rel_id`, `tblitems_in_div_con`.`division_id` AS `div`, `tblitems_in_div_con`.`contact_id` AS `con`, CONCAT(`tblcontacts`.`firstname`, " ", `tblcontacts`.`lastname`," (", `tblcontacts`.`title`,")"," (", `tblcontacts`.`phonenumber`,")") as `contact_name`, `tbldivision`.`division`');
    }
    $CI->db->from('tblitems_in_div_con');
    $CI->db->join('tblcontacts', 'tblitems_in_div_con.contact_id = tblcontacts.id', 'INNER');
    $CI->db->join('tbldivision', 'tblitems_in_div_con.division_id = tbldivision.id', 'INNER');
    if (isset($id)){
        $CI->db->where('rel_id', $id);
    }
    $CI->db->where('rel_type', $type);
...
//        $CI->db->group_end();
    ...
    $CI->db->order_by('contact_name', 'asc');
    return $CI->db->get()->result_array();
}

function add_new_div_con_item_post($div, $con, $rel_id, $rel_type)
{

    $CI = &get_instance();

    $CI->db->insert('tblitems_in_div_con', [
        'rel_id' => $rel_id,
        'rel_type' => $rel_type,
        'division_id' => $div,
        'contact_id' => $con
...
function handle_removed_div_cons_post($id, $rel_type)
{
    $CI = &get_instance();

    $CI->db->where('rel_id', $id);
    $CI->db->where('rel_type', $rel_type);
    $CI->db->delete('tblitems_in_div_con');

    return false;
}

function get_where_report_period($field = 'date')
{
    $CI = &get_instance();
    $months_report      = $CI->input->post('report_months');
    /*echo '<pre>';
    print_r($_POST);
    die();*/
    $custom_date_select = '';
    if ($months_report != '') {
        if (is_numeric($months_report)) {
            ...
            $from_date = to_sql_date($CI->input->post('report_from'));
            $to_date   = to_sql_date($CI->input->post('report_to'));
