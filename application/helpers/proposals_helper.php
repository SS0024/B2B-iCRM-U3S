 ...
 * @return string
 */
function format_inquiry_number($id)
{
    return get_option('inquiry_number_prefix') . str_pad($id, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
}
 ...
 * @return array
 */
function get_inquiry_item_taxes($itemid)
{
    $CI = & get_instance();
    $CI->db->where('itemid', $itemid);
    $CI->db->where('rel_type', 'inquiry');
    $taxes = $CI->db->get('tblitemstax')->result_array();
    $i     = 0;
    foreach ($taxes as $tax) {
        $taxes[$i]['taxname'] 		= $tax['taxname'] . '|' . $tax['taxrate'];
...
/**
 * Function that return tender item taxes based on passed item id
 * @param  mixed $itemid
 * @return array
 */
function get_tender_item_taxes($itemid)
{
    $CI = & get_instance();
    $CI->db->where('itemid', $itemid);
    $CI->db->where('rel_type', 'tender');
    $taxes = $CI->db->get('tblitemstax')->result_array();
    $i     = 0;
    foreach ($taxes as $tax) {
        $taxes[$i]['taxname'] 		= $tax['taxname'] . '|' . $tax['taxrate'];
 ...
 * @return array
 */
function get_inquiries_percent_by_status($status, $total_proposals = '')
{
    $has_permission_view                 = has_permission('proposals', '', 'view');
    $has_permission_view_own             = has_permission('proposals', '', 'view_own');
    $allow_staff_view_proposals_assigned = get_option('allow_staff_view_proposals_assigned');
    ...
    if (!is_numeric($total_proposals)) {
        $total_proposals = total_rows('tblinquiries', $whereUser);
    }
    ...
    $total_by_status = total_rows('tblinquiries', $where);
    $percent         = ($total_proposals > 0 ? number_format(($total_by_status * 100) / $total_proposals, 2) : 0);
...
function get_tenders_sql_where_staff($staff_id)
{
    $has_permission_view_own            = has_permission('tenders', '', 'view_own');
//    $allow_staff_view_invoices_assigned = get_option('allow_staff_view_proposals_assigned');
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '((tbltenders.addedfrom=' . $staff_id . ' AND tbltenders.addedfrom IN (SELECT staffid FROM tblstaffpermissions JOIN tblpermissions ON tblpermissions.permissionid=tblstaffpermissions.permissionid WHERE tblpermissions.name = "tenders" AND can_view_own=1))';
        /*if ($allow_staff_view_invoices_assigned == 1) {
            $whereUser .= ' OR assigned=' . $staff_id;
        }*/
        $whereUser .= ')';
