 ...
 * @return mixed  total left
 */
function get_purchaseorder_total_left_to_pay($id, $invoice_total = null)
{
    $CI = & get_instance();
    ...
    if ($invoice_total === null) {
        $CI->db->select('total')
        ->where('id', $id);
        $invoice_total = $CI->db->get('tblpurchaseorder')->row()->total;
    }

    if (!class_exists('purchaseorder_payments_model')) {
        $CI->load->model('purchaseorder_payments_model');
    }

    $payments = $CI->purchaseorder_payments_model->get_invoice_payments($id);
...
function get_service_invoice_total_left_to_pay($id, $invoice_total = null)
{
    $CI = & get_instance();
    ...
    if ($invoice_total === null) {
        $CI->db->select('total')
            ->where('id', $id);
        $invoice_total = $CI->db->get('tbliservicenvoices')->row()->total;
    }

    if (!class_exists('invoice_payments_model')) {
        $CI->load->model('invoice_payments_model');
    }
    ...
    $payments = $CI->invoice_payments_model->get_invoice_payments($id);
    $credits  = $CI->credit_notes_model->get_applied_invoice_credits($id);
 ...
 * @return mixed
 */
function format_delivery_challan_status($status, $classes = '', $label = true)
{
    $id          = $status;
    $label_class = get_invoice_status_label($status);
    if ($status == 1) {
...
function update_service_invoice_status($id, $force_update = false, $prevent_logging = false)
{
    $CI = & get_instance();

    $CI->load->model('service_invoices_model');
    $invoice = $CI->service_invoices_model->get($id);
    ...
    $CI->db->select('amount')
        ->where('invoiceid', $id)
        ->order_by('tblinvoiceservicepaymentrecords.id', 'asc');
    $payments = $CI->db->get('tblinvoiceservicepaymentrecords')->result_array();
    ...
    $CI->db->where('id', $id);
    $CI->db->update('tblserviceinvoices', [
        'status' => $status,
    ]);
    ...
    if ($CI->db->affected_rows() > 0) {
        ...
        $log = 'Service Invoice Status Updated [Invoice Number: ' . format_service_invoice_number($invoice->id) . ', From: ' . format_invoice_status($original_status, '', false) . ' To: ' . format_invoice_status($status, '', false) . ']';
 ...
 * @return string
 */
function format_delivery_challan_number($id)
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from('tbldelivery_challans')->where('id', $id);
    $invoice = $CI->db->get()->row();
 ...
 * @return string
 */
function format_service_invoice_number($id)
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from('tblserviceinvoices')->where('id', $id);
    $invoice = $CI->db->get()->row();
    ...
    $hook_data['invoice']          = $invoice;
    $hook_data['formatted_number'] = $number;
    $hook_data                     = do_action('format_service_invoice_number', $hook_data);
    $number                        = $hook_data['formatted_number'];

    return $number;
}

function format_service_pi_number($id)
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from('tblservicepi')->where('id', $id);
    $invoice = $CI->db->get()->row();
    ...
    $hook_data['invoice']          = $invoice;
    $hook_data['formatted_number'] = $number;
    $hook_data                     = do_action('format_service_pi_number', $hook_data);
    $number                        = $hook_data['formatted_number'];
...
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from('tblpurchases')->where('id', $id);
    $invoice = $CI->db->get()->row();
    ...
    $hook_data['invoice']          = $invoice;
    $hook_data['formatted_number'] = $number;
    $hook_data                     = do_action('format_purchase_number', $hook_data);
    $number                        = $hook_data['formatted_number'];

    return $number;
}

function get_delivery_challan_item_taxes($itemid)
{
    $CI = & get_instance();
    $CI->db->where('itemid', $itemid);
    $CI->db->where('rel_type', 'deliverychallan');
...
function get_service_invoice_item_taxes($itemid)
{
    $CI = & get_instance();
    $CI->db->where('itemid', $itemid);
    $CI->db->where('rel_type', 'service_invoice');
    $taxes = $CI->db->get('tblitemstax')->result_array();
    $i     = 0;
    foreach ($taxes as $tax) {
        $taxes[$i]['taxname'] = $tax['taxname'] . '|' . $tax['taxrate'];
...
function get_service_pi_item_taxes($itemid)
{
    $CI = & get_instance();
    $CI->db->where('itemid', $itemid);
    $CI->db->where('rel_type', 'service_pi');
    $taxes = $CI->db->get('tblitemstax')->result_array();
    $i     = 0;
    foreach ($taxes as $tax) {
        $taxes[$i]['taxname'] = $tax['taxname'] . '|' . $tax['taxrate'];
...
function is_payment_mode_allowed_for_service_invoice($id, $invoiceid)
{
    $CI = & get_instance();
    $CI->db->select('tblcurrencies.name as currency_name,allowed_payment_modes')->from('tblserviceinvoices')->join('tblcurrencies', 'tblcurrencies.id = tblserviceinvoices.currency', 'left')->where('tblserviceinvoices.id', $invoiceid);
    $invoice       = $CI->db->get()->row();
    $allowed_modes = $invoice->allowed_payment_modes;
    if (!is_null($allowed_modes)) {
        $allowed_modes = unserialize($allowed_modes);
...
function get_purchase_where_sql_for_staff($staff_id)
{
    $has_permission_view_own            = has_permission('purchases', '', 'view_own');
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '((tblpurchases.addedfrom=' . $staff_id . ' AND tblpurchases.addedfrom IN (SELECT staffid FROM tblstaffpermissions JOIN tblpermissions ON tblpermissions.permissionid=tblstaffpermissions.permissionid WHERE tblpermissions.name = "purchases" AND can_view_own=1))';
...
function get_servicepi_where_sql_for_staff($staff_id)
{
    $has_permission_view_own            = has_permission('service_pi', '', 'view_own');
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '((tblservicepi.addedfrom=' . $staff_id . ' AND tblservicepi.addedfrom IN (SELECT staffid FROM tblstaffpermissions JOIN tblpermissions ON tblpermissions.permissionid=tblstaffpermissions.permissionid WHERE tblpermissions.name = "service_pi" AND can_view_own=1))';
        $whereUser .= ' OR sale_agent=' . $staff_id;
        $whereUser .= ')';
    } else {
        $whereUser .= 'sale_agent=' . $staff_id;
...
function get_service_invoices_where_sql_for_staff($staff_id)
{
    $has_permission_view_own            = has_permission('service_invoices', '', 'view_own');
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '((tblserviceinvoices.addedfrom=' . $staff_id . ' AND tblserviceinvoices.addedfrom IN (SELECT staffid FROM tblstaffpermissions JOIN tblpermissions ON tblpermissions.permissionid=tblstaffpermissions.permissionid WHERE tblpermissions.name = "service_invoices" AND can_view_own=1))';
        $whereUser .= ' OR sale_agent=' . $staff_id;
        $whereUser .= ')';
    } else {
        $whereUser .= 'sale_agent=' . $staff_id;
...
function get_delivery_challan_where_sql_for_staff($staff_id)
{
    $has_permission_view_own            = has_permission('delivery_challan', '', 'view_own');
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '((tbldelivery_challans.addedfrom=' . $staff_id . ' AND tbldelivery_challans.addedfrom IN (SELECT staffid FROM tblstaffpermissions JOIN tblpermissions ON tblpermissions.permissionid=tblstaffpermissions.permissionid WHERE tblpermissions.name = "delivery_challan" AND can_view_own=1))';
        $whereUser .= ' OR sale_agent=' . $staff_id;
        $whereUser .= ')';
    } else {
        $whereUser .= 'sale_agent=' . $staff_id;
