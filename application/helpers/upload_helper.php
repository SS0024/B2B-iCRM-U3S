 ...
 * @return boolean
 */
function handle_lead_attachments($leadid, $index_name = 'file', $form_activity = false,$profile_type = '')
{
    ...
    if (isset($_FILES[$index_name]['name']) && $_FILES[$index_name]['name'] != '') {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->leads_model->add_attachment_to_database($leadid, $data, false, $form_activity,$profile_type);
...
function get_upload_path_by_type($type)
{
    switch ($type) {
        ...
        break;
        case 'inquiry':
            return INQUIRY_ATTACHMENTS_FOLDER;

        break;
        case 'requsite_form':
            return REQUSITE_FORM_ATTACHMENTS_FOLDER;

        break;
        case 'purchaseorder':
            return PO_ATTACHMENTS_FOLDER;
		...
		break;
        case 'delivery_modules':
        return DELIVERY_MODULES_FOLDER;
		
		break;
        case 'invoice_payment':
        return INVOICE_PAYMENT_FOLDER;
		
		break;
        case 'invoice_payment_temp':
        return INVOICE_PAYMENT_TEMP_FOLDER;
...
/**
 * Handles upload for delivery module attachment
 * @param  mixed $id delivery_module id
 * @return void
 */
function handle_delivery_module_attachments($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    $path = get_upload_path_by_type('delivery_modules') . $id . '/';
	
    $CI   = & get_instance();

    if (isset($_FILES['file']['name'])) {
        do_action('before_upload_delivery_modules_attachment', $id);
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'delivery_modules', $attachment);
            }
        }
    }
}
...
/**
 * Handles upload for handle_invoice_payment_attachments
 * @param  mixed $id invoice_payment record id
 * @return void
 */
function handle_temp_invoice_payment_attachments()
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    $path = get_upload_path_by_type('invoice_payment_temp');
	
    $CI   = & get_instance();

    if (isset($_FILES['file']['name'])) {
        do_action('before_upload_invoice_payment_attachment');
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if(move_uploaded_file($tmpFilePath, $newFilePath)){
                ...
                $id = $CI->misc_model->add_attachment_to_temp_database('invoice_payment', $attachment);
				
				$return_data = array();
				$return_data = $attachment[0];
				$return_data['id'] = $id;
				return $return_data;
            }
        }
