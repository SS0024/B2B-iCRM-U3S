...
{
    $CI      = & get_instance();
    $project = $CI->object_cache->get('division-name-data-' . $id);

    if (!$project) {
        $CI->db->select('division as name');
        $CI->db->where('id', $id);
        $project = $CI->db->get('tbldivision')->row();
        $CI->object_cache->add('division-name-data-' . $id, $project);
    }
 ...
 * @return boolean
 */
function customer_has_divisions($customer_id)
{
    $totalCustomerProjects = total_rows('tbldivision', 'userid=' . $customer_id);
