 ...
 * @return string
 */
function format_estimate_number_pi($id)
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from('tblestimates')->where('id', $id);
    $estimate = $CI->db->get()->row();
...
function format_requsite_form_number($id)
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from('tblrequsiteform')->where('id', $id);
    $estimate = $CI->db->get()->row();
...
function get_all_estimate_item_taxes($rel_id)
{
    $CI = & get_instance();
 ...
 * @return array
 */
function get_requisite_forms_percent_by_status($status, $project_id = null)
{
    ...
    if (!$has_permission_view) {
        $where .= get_requsite_form_where_sql_for_staff(get_staff_user_id());
    }
    ...
    $total_estimates = total_rows('tblrequsiteform', $where);
    ...
    if (!is_numeric($status)) {
        if ($status == 'not_sent') {
            $total_by_status = total_rows('tblrequsiteform', 'sent=0 AND status NOT IN(2,3,4)' . ($where != '' ? ' AND (' . $where . ')' : ''));
        }
    } else {
        $whereByStatus = 'status=' . $status;
        if ($where != '') {
        ...
        $total_by_status = total_rows('tblrequsiteform', $whereByStatus);
    }
 ...
 * @return array
 */
function get_purchaseorder_percent_by_status($status, $project_id = null)
{
    ...
    if (!$has_permission_view) {
        $where .= get_purchaseorder_where_sql_for_staff(get_staff_user_id());
    }
...
function get_requsite_form_where_sql_for_staff($staff_id)
{
    $has_permission_view_own             = has_permission('requisite_forms', '', 'view_own');
    $whereUser                           = '';
    if ($has_permission_view_own) {
        $whereUser = '((tblrequsiteform.addedfrom=' . $staff_id . ' AND tblrequsiteform.addedfrom IN (SELECT staffid FROM tblstaffpermissions JOIN tblpermissions ON tblpermissions.permissionid=tblstaffpermissions.permissionid WHERE tblpermissions.name = "requisite_forms" AND can_view_own=1))';
        $whereUser .= ' OR sale_agent=' . $staff_id;
        $whereUser .= ')';
    } else {
        $whereUser .= 'sale_agent=' . $staff_id;
...
function get_purchaseorder_where_sql_for_staff($staff_id)
{
    $has_permission_view_own             = has_permission('purchaseorder', '', 'view_own');
    $whereUser                           = '';
    if ($has_permission_view_own) {
