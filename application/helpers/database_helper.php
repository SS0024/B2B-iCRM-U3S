 ...
 */
function get_staff_full_name($userid = '', $displayContact=false)
{
    ...
    if (!$staff) {
        ...
        $staff = $CI->db->select('firstname,lastname,phonenumber')->from('tblstaff')->get()->row();
        $CI->object_cache->add('staff-full-name-data-' . $userid, $staff);
    }
    return $staff ? $staff->firstname . ' ' . $staff->lastname.($displayContact===true && $staff->phonenumber ? " ({$staff->phonenumber})" : '' ) : '';
}
