...
function get_stock_item_details_from_code($itemCode){
    $CI = & get_instance();
    $CI->db->select('*');
    $CI->db->from('tblitems');
    $q = $CI->db->where('description' , $itemCode)->get();
    $item = $q->row();
    return (int)$item->quantity - (int) $item->hold_stock;
}


/*function plus_item_hold_quantity($itemCode,$qty){
    $CI = & get_instance();
    $CI->db->where('description', $itemCode);
    $CI->db->set('hold_stock', 'hold_stock+'.$qty, false);
    $CI->db->update('tblitems');
    return true;
}*/

function add_hold_item($item, $rel_id, $rel_type){
    $CI = &get_instance();
    $product_id = 0;
    if(isset($item['description']) && !isset($item['id']))
    {
        $CI->db->select('id');
        $CI->db->from('tblitems');
        $q = $CI->db->where('description' , $item['description'])->get();
        if ($q->num_rows() > 0) {
            $product_id = $q->row()->id;
        }else{
            return FALSE;
    ...
    $id = $CI->db->insert_id();
    $q = $CI->db->query('SELECT sum(qty) as hold_qty FROM `tblhold_items` WHERE product_id  = ?', [$product_id]);
    if ($q->num_rows() > 0) {
        $res =$q->row();
        $CI->db->where('id', $product_id);
        $CI->db->update('tblitems',['hold_stock',$res->hold_qty]);
    }else{
        $CI->db->where('id', $product_id);
        $CI->db->update('tblitems',['hold_stock'=>'0']);
    }
}

/*function minus_item_hold_quantity($itemCode,$qty){
    $CI = & get_instance();
    $CI->db->where('description', $itemCode);
    $CI->db->set('hold_stock', 'hold_stock-'.$qty, false);
    $CI->db->update('tblitems');
...
function remove_hold_item($item, $rel_id, $rel_type){
    $CI = &get_instance();
    $product_id = 0;
    if(isset($item['description']) && !isset($item['id']))
    {
    ...
    $CI->db->delete('tblhold_items');
    $q = $CI->db->query('SELECT sum(qty) as hold_qty FROM `tblhold_items` WHERE product_id  = ?', [$product_id]);
    if ($q->num_rows() > 0 && isset($q->row()->hold_qty)) {
        $res =$q->row();
        $CI->db->where('id', $product_id);
        $CI->db->set('hold_stock', $res->hold_qty, false);
        $CI->db->update('tblitems');
    }else{
        $CI->db->where('id', $product_id);
        $CI->db->update('tblitems',['hold_stock'=>'0']);
...
function get_type_of_machines()
{
    $CI = &get_instance();
    if (!class_exists('Scheduled_services_model')) {
        $CI->load->model('Scheduled_services_model');
    }
    $machines = $CI->Scheduled_services_model->get_machines();
    return $machines;
}