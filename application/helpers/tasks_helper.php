...
function task_rel_name($rel_name, $rel_id, $rel_type)
{
    ...
    } elseif ($rel_type == 'inquiry') {
        $rel_name = format_inquiry_number($rel_id);
    }elseif ($rel_type == 'tender') {
        $this->db->select('tender_number');
        $this->db->from('tbltenders');
        $this->db->where('id', $rel_id);
        $proposal = $this->db->get()->row();
        $rel_name = $proposal->tender_number;
    }
...
function task_rel_link($rel_id, $rel_type)
{
    ...
    if ($rel_type == 'customer') {
        ...
        $link = admin_url('inquiries/list_inquiries/' . $rel_id);
    }elseif ($rel_type == 'tender') {
        $link = admin_url('tenders/list_tenders/' . $rel_id);
    }
...
function tasks_rel_name_select_query()
{
    return '(CASE rel_type
        ...
        WHEN "inquiry" THEN (SELECT id FROM tblinquiries WHERE tblinquiries.id = tblstafftasks.rel_id)
