...
function get_relation_data($type, $rel_id = '', $connection_type = '', $connection_id = '')
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($rel_id != '') {
            $CI->load->model('inquiries_model');
            $data = $CI->inquiries_model->get($rel_id);
        } else {
            $search = $CI->misc_model->_search_inquiries($q);
            $data   = $search['result'];
        }
    }   elseif ($type == 'tender') {
        if ($rel_id != '') {
            $CI->load->model('tenders_model');
            $data = $CI->tenders_model->get($rel_id);
        } else {
            $search = $CI->misc_model->_search_tenders($q);
            $data   = $search['result'];
        }
    } elseif ($type == 'project') {
        if ($rel_id != '') {
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        $name = format_inquiry_number($id);
        $link = admin_url('inquiries/inquiry/' . $id);
    }  elseif ($type == 'tender') {
        if (is_array($relation)) {
            $name = $relation['tender_number'];
            $id        = $relation['id'];
            $addedfrom = $relation['addedfrom'];
            if (!empty($relation['subject'])) {
                $name .= ' - ' . $relation['subject'];
        ...
        $link = admin_url('tenders/tender/' . $id);
    } elseif ($type == 'tasks') {
        if (is_array($relation)) {
            $id   = $relation['id'];
            $name = $relation['name'];
