...
function invoice_pdf($invoice, $tag = '')
{
	...
	if ($CI->input->get('type') == 'way_bill') {
		include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/way_bill_invoicepdf.php');
	}else{
 ...
 * @return mixed object
 */
function delivery_challan_pdf($invoice, $tag = '')
{
    $GLOBALS['invoice_pdf'] = $invoice;
    $CI                     = & get_instance();
    load_pdf_language($invoice->clientid);
	...
	if ($CI->input->get('type') == 'way_bill') {
		...
		if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_invoicepdf.php')) {
			...
			include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/delivery_challan_pdf.php');
		}
	}
...
function service_invoice_pdf($invoice, $tag = '')
{
    $GLOBALS['invoice_pdf'] = $invoice;
    $CI                     = & get_instance();
    load_pdf_language($invoice->clientid);
    ...
    if ($CI->input->get('type') == 'way_bill') {
        ...
        if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_invoicepdf.php')) {
            ...
            include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/service_invoicepdf.php');
        }
    }
...
function service_pi_pdf($invoice, $tag = '')
{
    $GLOBALS['invoice_pdf'] = $invoice;
    $CI                     = & get_instance();
    load_pdf_language($invoice->clientid);
    ...
    if ($CI->input->get('type') == 'way_bill') {
        ...
        if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_invoicepdf.php')) {
            ...
            include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/service_pipdf.php');
        }
    }
...
function stock_in_pdf($invoice, $tag = '')
{
    $GLOBALS['invoice_pdf'] = $invoice;
    $CI                     = & get_instance();
    $CI->load->library('pdf');
    ...
    include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/stock_in_pdf.php');
...
function stock_out_extra_pdf($invoice, $tag = '')
{
    $GLOBALS['invoice_pdf'] = $invoice;
    $CI                     = & get_instance();
    $CI->load->library('pdf');
    ...
    include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/stock_out_extra_pdf.php');
...
function stock_out_pdf($invoice, $tag = '')
{
    $GLOBALS['invoice_pdf'] = $invoice;
    $CI                     = & get_instance();
    $CI->load->library('pdf');
    ...
    include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/stock_out_pdf.php');
...
/**
 * Prepare general purchaseorder pdf
 * @since  Version 1.0.2
 * @param  object $purchaseorder purchaseorder as object with all necessary fields
 * @param  string $tag tag for bulk pdf exporter
 * @return mixed object
 */
function purchaseorder_pdf($estimate, $tag = '')
{
...
function estimate_pdf($estimate, $tag = '')
{
    ...
    load_pdf_language($estimate->clientid);
    $CI->load->library('pdf');
    if (isset($estimate->is_pi) && $estimate->is_pi){
        $estimate_number = format_estimate_number_pi($estimate->id);
    }else{
        $estimate_number = format_estimate_number_pi($estimate->id);
    }
    $font_name       = get_option('pdf_font');
...
function requsite_form_pdf($estimate, $tag = '')
{
    $GLOBALS['estimate_pdf'] = $estimate;
    $CI                      = & get_instance();
    load_pdf_language($estimate->clientid);
    ...
    $CI->load->model('requsite_form_model');
    $statuses=$CI->requsite_form_model->get_statuses();
    $status = $statuses[$estimate->status];
    $swap   = get_option('swap_pdf_info');
    $CI->load->library('numberword', [
        'clientid' => $estimate->clientid,
    ]);
    ...
    if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_estimatepdf.php')) {
        ...
        include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/requsite_form_pdf.php');
    }
...
function proposal_pdf($proposal, $tag = '')
{
    ...
    $number       = format_inquiry_number($proposal->id);
...
function service_payment_pdf($payment, $tag = '')
{
    $GLOBALS['payment_pdf'] = $payment;
    $CI                     = & get_instance();
    load_pdf_language($payment->invoice_data->clientid);
    ...
    if (file_exists(APPPATH . 'views/themes/' . active_clients_theme() . '/views/my_paymentpdf.php')) {
        ...
        include(APPPATH . 'views/themes/' . active_clients_theme() . '/views/service_paymentpdf.php');
    }
