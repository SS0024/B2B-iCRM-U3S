...
class Contracts extends Admin_controller
{
    ...
    public function contract($id = '')
    {
        if ($this->input->post()) {
            if ($id == '') {
                ...
                if (isset($data['item_id']) && !empty($data['item_id'])) {
                    ...
                    $this->db->update('tblitems_in', ['type_of_machine' => $data['contract_type']]);
                }
        ...
        if ($this->input->get('product_id') != '' && $this->input->get('category_type') != '' || isset($data['contract']->item_id)) {
            $this->db->join('tblinvoices', 'tblinvoices.id = tblitems_in.rel_id', 'left');
            $this->db->where('tblitems_in.id', ($this->input->get('product_id') != '' ? $this->input->get('product_id') : $data['contract']->item_id));
            $item = $this->db->get('tblitems_in')->row();
            $data['item_details'] = $item;
            $data['category_type'] = $this->input->get('category_type');
            $data['subject'] = $item->fab_no;
            $data['customer_id'] = isset($item->clientid) && !empty($item->clientid) ? $item->clientid : (isset($item->userId) && !empty($item->userId) ? $item->userId : 0);
        }
        $this->load->model('currencies_model');
        $data['base_currency'] = $this->currencies_model->get_base_currency();
