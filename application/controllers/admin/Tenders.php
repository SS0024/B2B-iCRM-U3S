...
class Tenders extends Admin_controller
{
    ...
    {
        $this->list_tenders($proposal_id);
    }

    public function list_tenders($proposal_id = '')
    {
        close_setup_menu();

        if (!has_permission('tenders', '', 'view') && !has_permission('tenders', '', 'view_own')) {
            access_denied('tenders');
        }
        ...
        if ($isPipeline && !$this->input->get('status')) {
            ...
            $this->load->view('admin/tenders/pipeline/manage', $data);
        } else {
            ...
            $data['title'] = _l('tender');
            $data['statuses'] = $this->tenders_model->get_statuses();
            $data['proposals_sale_agents'] = $this->tenders_model->get_sale_agents();
            $data['years'] = $this->tenders_model->get_proposals_years();
            $this->load->view('admin/tenders/manage', $data);
        }
    }
    ...
    {
        $this->app->get_table_data('tenders');
    }
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->tenders_model->delete_attachment($id);
        } else {
            ajax_access_denied();
        }
    }
    ...
    {
        if (has_permission('tenders', '', 'delete')) {
            $this->tenders_model->clear_signature($id);
        }

        redirect(admin_url('tenders/list_tenders/' . $id));
    }

    public function sync_data()
    {
        if (has_permission('tenders', '', 'create') || has_permission('tenders', '', 'edit')) {
            $has_permission_view = has_permission('tenders', '', 'view');
    ...
    public function tender($id = '')
    {
        if ($this->input->post()) {
            ...
            unset($proposal_data['master_file']);
            if (!empty($proposal_data['master_files'])) {
                $proposal_data['master_files'] = implode(', ', $proposal_data['master_files']);
            }
            if ($id == '') {
                if (!has_permission('tenders', '', 'create')) {
                    access_denied('tenders');
                }
                $id = $this->tenders_model->add($proposal_data);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('tender')));
                ...
                if (!has_permission('tenders', '', 'edit')) {
                    access_denied('tenders');
                }
                $success = $this->tenders_model->update($proposal_data, $id);
                if ($success) {
                    set_alert('success', _l('updated_successfully', _l('tender')));
                }
                if ($this->set_proposal_pipeline_autoload($id)) {
                    redirect(admin_url('inquiries'));
                } else {
        ...
        if ($id == '') {
            $title = _l('add_new', _l('tender_lowercase'));
        } else {
            $data['proposal'] = $this->tenders_model->get($id);
            ...
            $title = _l('edit', _l('tender_lowercase'));
        }
        ...
        $data['bodyclass'] = 'tender';

        $data['title'] = $title;
        $this->load->view('admin/tenders/proposal', $data);
    }
    ...
    public function get_template()
    {
        ...
        echo $this->load->view('admin/tenders/templates/' . $name, [], true);
    }
    ...
    public function send_expiry_reminder($id)
    {
        ...
        $success = $this->tenders_model->send_expiry_reminder($id);
        if ($success) {
    ...
    public function clear_acceptance_info($id)
    {
        if (is_admin()) {
            $this->db->where('id', $id);
            $this->db->update('tbltenders', get_acceptance_info_array(true));
        }
    ...
    public function pdf($id)
    {
        ...
        $proposal = $this->tenders_model->get($id);
    ...
    public function add_note($rel_id)
    {
        if ($this->input->post() && user_can_view_proposal($rel_id)) {
            $this->misc_model->add_note($this->input->post(), 'tender', $rel_id);
            echo $rel_id;
        }
    }
    ...
    public function get_notes($id)
    {
        if (user_can_view_proposal($id)) {
            $data['notes'] = $this->misc_model->get_notes($id, 'tender');
    ...
    public function get_invoice_convert_data($id)
    {
        ...
        $this->load->view('admin/tenders/invoice_convert_template', $data);
    }
    ...
    public function get_estimate_convert_data($id)
    {
        ...
        $this->load->view('admin/tenders/estimate_convert_template', $data);
    }
    ...
    public function send_to_email($id)
    {
        ...
        if ($this->input->post()) {
            try {
                $success = $this->tenders_model->send_proposal_to_email($id, 'proposal-send-to-customer', $this->input->post('attach_pdf'), $this->input->post('cc'));
            } catch (Exception $e) {
                $message = $e->getMessage();
                echo $message;
                if (strpos($message, 'Unable to get the size of the image') !== false) {
    ...
    public function mark_action_status($status, $id)
    {
        ...
        $success = $this->tenders_model->mark_action_status($status, $id);
        if ($success) {
            set_alert('success', _l('Tender status changed successfully'));
        } else {
            set_alert('danger', _l('Failed to change tender status'));
        }
        if ($this->set_proposal_pipeline_autoload($id)) {
    ...
    public function delete($id)
    {
        ...
        $response = $this->tenders_model->delete($id);
        if ($response == true) {
            set_alert('success', _l('deleted', _l('tender')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('inquiry_lowercase')));
        }
        redirect(admin_url('inquiries'));
    }
    ...
    {
        echo json_encode($this->tenders_model->get_relation_data_values($rel_id, $rel_type));
    }
    ...
    public function add_proposal_comment()
    {
        if ($this->input->post()) {
            echo json_encode([
                'success' => $this->tenders_model->add_comment($this->input->post()),
            ]);
        }
    }
    ...
    public function edit_comment($id)
    {
        if ($this->input->post()) {
            echo json_encode([
                'success' => $this->tenders_model->edit_comment($this->input->post(), $id),
    ...
    {
        $data['comments'] = $this->tenders_model->get_comments($id);
        $this->load->view('admin/tenders/comments_template', $data);
    }
    ...
    public function remove_comment($id)
    {
        ...
        if ($comment) {
            ...
            echo json_encode([
                'success' => $this->tenders_model->remove_comment($id),
            ]);
        } else {
            echo json_encode([
                'success' => false,
    ...
    {
        if (!has_permission('tenders', '', 'edit') && !has_permission('tenders', '', 'create')) {
        ...
        $message = _l('updated_successfully', _l('tender'));
    ...
    public function pipeline_open($id)
    {
        if (has_permission('tenders', '', 'view') || has_permission('tenders', '', 'view_own')) {
            ...
            $data['proposal_data'] = $this->tenders_model->get($id);
            $this->load->view('admin/tenders/pipeline/proposal', $data);
        }
    }
    ...
    public function get_proposal_data_ajax($id, $to_return = false)
    {
        ...
        $proposal = $this->tenders_model->get($id, [], true);
        ...
        $data['proposal_statuses'] = $this->tenders_model->get_statuses();
        $data['members'] = $this->staff_model->get('', ['active' => 1]);
        $data['activity'] = $this->tenders_model->get_estimate_activity($id);
        $data['proposal_merge_fields'] = $_proposal_merge_fields;
        $data['proposal'] = $proposal;
        $data['totalNotes'] = total_rows('tblnotes', ['rel_id' => $id, 'rel_type' => 'tender']);
        if ($to_return == false) {
            $this->load->view('admin/tenders/proposals_preview_template', $data);
        } else {
            return $this->load->view('admin/tenders/proposals_preview_template', $data, true);
        }
    }
    ...
    {
        if (has_permission('tenders', '', 'edit')) {
            $this->tenders_model->update_pipeline($this->input->post());
        }
    }
    ...
    public function get_pipeline()
    {
        if (has_permission('tenders', '', 'view') || has_permission('tenders', '', 'view_own')) {
            $data['statuses'] = $this->tenders_model->get_statuses();
            $this->load->view('admin/tenders/pipeline/pipeline', $data);
        }
    }
    ...
    public function pipeline_load_more()
    {
        ...
        $proposals = $this->tenders_model->do_kanban_query($status, $this->input->get('search'), $page, [
            'sort_by' => $this->input->get('sort_by'),
            'sort' => $this->input->get('sort'),
        ]);

        foreach ($proposals as $proposal) {
            $this->load->view('admin/tenders/pipeline/_kanban_card', [
                'tender' => $proposal,
                'status' => $status,
            ]);
        }
