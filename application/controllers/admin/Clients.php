...
class Clients extends Admin_controller
{
    ...
    public function all_divisions()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('all_division');
        }
        ...
        $data['title'] = _l('Divisions');
        $data['divisions'] = $this->clients_model->get_divisions();
        //$this->load->view('admin/clients/all_contacts', $data);
        $this->load->view('admin/clients/all_divisions', $data);

        /*$data['divisions'] = $this->clients_model->get_all_division();
        $data['title'] = _l('Divisions');
        $this->load->view('admin/clients/all_divisions', $data);*/
    }
    ...
    public function client($id = '')
    {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    if ($save_and_add_contact == false) {
                        ...
                        redirect(admin_url('clients/client/' . $id . '?group=division&new_division=true'));
                    }
                }
            } else {
        ...
        if ($id == '') {
            ...
            $data['divisions'] = $this->clients_model->get_divisions($id);
            ...
            if ($group == 'profile') {
                ...
                if (!has_permission('my_products', '', 'view')) {
    ...
    public function validate_gst_exist()
    {
        $isedit          = $this->input->post('isedit');
        $description = $this->input->post('vat');
        if ($isedit) {
            $original_number = $this->clients_model->get($isedit);

            if (isset($original_number->vat) && $description == $original_number->vat) {
    ...
    public function contact($customer_id, $contact_id = '')
    {
        ...
        $data['divisions'] = $this->clients_model->get_divisions($customer_id);
        if ($this->input->post()) {
            $data = $this->input->post();
    ...
    public function division($customer_id, $contact_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '')
            {
                ...
                $id      = $this->clients_model->add_division($data, $customer_id);
                $message = '';
                $success = false;
                ...
                echo json_encode([
                    'success'             => $success,
                    'message'             => $message,
                    'is_individual'       => is_empty_customer_company($customer_id) && total_rows('tbldivision', ['userid' => $customer_id]) == 1,
                ]);
                die;
            ...
            $success          = $this->clients_model->update_division($data, $contact_id);
            $message          = '';
            $proposal_warning = false;
            $original_email   = '';
            $updated          = false;
            if ($updated == true) {
                $contact = $this->clients_model->get_divisions($customer_id);
            }
            echo json_encode([
                'success'             => $success,
                'proposal_warning'    => $proposal_warning,
                'message'             => $message,
                'original_email'      => $original_email,
                'has_primary_contact' => (total_rows('tbldivision', ['userid' => $customer_id]) > 0 ? true : false),
            ]);
            die;
        }

        if ($contact_id == '') {
            $title = _l('add_new', 'division');
        } else {

            $dgd=$this->clients_model->get_divisions($customer_id);
            $oo='';
            $i=0;
            foreach ($_REQUEST as $key => $valuesg)
            {
                if($i==0)
                {
                    $fp=explode("/",$key);
            ...
            $pp=[];
            $data['divisions'] = $this->clients_model->get_divisions($customer_id);
            foreach ($dgd as $keys => $values)
            {
                if($values->id==$oo)
                {
                    array_push($pp,$values);
                }
            }
            if (!$data['divisions']) {
                header('HTTP/1.0 400 Bad error');
            ...
            $title = $data['divisions'][0]->division;
        }
        $data['customer_permissions'] = get_contact_permissions();
        $data['title']                = $pp->division;
        $data['divisions'] = $pp;
        $this->load->view('admin/clients/modals/division', $data);

    }

    public function product($customer_id)
    {
        if (!has_permission('my_products', '', 'create') && !has_permission('my_products', '', 'edit')) {
            access_denied('my_products');
        }
        if ($this->input->post()) {
            $data = $this->input->post();
            ...
            $newData = array(
                ...
                'custom_invoice' => $data['invoice_custom'],
            ...
            if(isset($data['id']) && !empty($data['id'])){
                ...
                $this->db->update('tblitems_in', $newData);
                if (isset($custom_fields)) {
                    handle_custom_fields_post($data['id'], $custom_fields, true);
                }
                $message = "Product is updated.";
            }else{
                $this->db->insert('tblitems_in', $newData);
    ...
    public function delete_product($productId)
    {
        ...
        handle_removed_sales_item_post($productId, 'invoice');
    ...
    public function divisions()
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                ...
                $id = $this->clients_model->add_division($data);
                if (!$inline) {
                    if ($id) {
                        set_alert('success', _l('added_successfully', _l('Division')));
                    }
                } else {
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->clients_model->update_division($data, $id);
                if ($success) {
                    set_alert('success', _l('updated_successfully', _l('Division')));
                }
            }
        }
    }
    ...
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            echo json_encode($this->clients_model->search($this->input->post('q')));
        }
    }
    ...
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            echo json_encode($this->clients_model->searchDivisions($this->input->post('q'),$this->input->post('customer_id')));
        }
    }

    public function get_contact($id)
    {
        echo json_encode($this->clients_model->get_contact($id));
    }

    public function loadcustominfo($id)
    {
        $this->db->select('*');
        $this->db->from('tblclients');
        $this->db->order_by('userid', 'asc');
        $this->db->where('userid',$id);
        $customerinfo = $this->db->get()->row();

        $cntryinfo=$this->db->query("select * from tblcountries where country_id='".$customerinfo->country."'")->row();
        $customerinfo->country=$cntryinfo->short_name;
        $oo=json_encode($customerinfo);
        echo $oo;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete('tbldivision');
        redirect(admin_url('clients/all_divisions'));
    }

    public function generateDivision(){
        $clients = $this->clients_model->get();
        foreach ($clients as $client){
            $division = $this->clients_model->get_divisions($client['userid']);
            if(count($division) < 1){
                $this->clients_model->add_division([
