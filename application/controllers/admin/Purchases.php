...
class Purchases extends Admin_controller
{
    ...
    {
        $this->list_purchases($id);
    }

    /* List all invoices datatables */
    public function list_purchases($id = '')
    {
        if (!has_permission('purchases', '', 'view')
            && !has_permission('purchases', '', 'view_own')) {
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != 5) ? $this->purchases_model->check_for_merge_invoice($customer_id, $current_invoice) : [];

            $data['merge_info'] = $this->load->view('admin/invoices/merge_invoice', $_data, true);

            $this->load->model('currencies_model');

            $__data['expenses_to_bill'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != 5) ? $this->purchases_model->get_expenses_to_bill($customer_id) : [];
    ...
    public function stock_in($purchase_id)
    {
        ...
        $invoice = $this->purchases_model->get($purchase_id);
        if ($this->input->post()) {
            $products = [];
            ...
            $newDate = $products;
            $success = $this->purchases_model->updateQuantityBalance($newDate, $invoice->warehouse_id);
            $this->db->where('id', $purchase_id);
            $this->db->update('tblpurchases', ['is_stock_in'=>2]);
            set_alert('success', 'Stock in successfully.');
            redirect(admin_url('purchases/list_purchases/'.$invoice->id));
    ...
    public function pause_overdue_reminders($id)
    {
        ...
        redirect(admin_url('invoices/list_purchases/' . $id));
    }
    ...
    public function mark_as_cancelled($id)
    {
        ...
        $success = $this->purchases_model->mark_as_cancelled($id);

        if ($success) {
            set_alert('success', _l('invoice_marked_as_cancelled_successfully'));
        }

        redirect(admin_url('purchases/list_purchases/' . $id));
    }
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $success = $this->purchases_model->unmark_as_cancelled($id);
        if ($success) {
    ...
    public function copy($id)
    {
        ...
        $new_id = $this->purchases_model->copy($id);
        if ($new_id) {
    ...
    {
        $invoice = $this->purchases_model->get($id);
...
//			$invoice_data['service_charge_tax_rate']	=	SERVICE_CHARGES_TAX_RATE;
            ...
            if ($id == '') {
				...
				//print_r($invoice_data);die;
                $id = $this->purchases_model->add($invoice_data);
                if ($id) {
                ...
                $success = $this->purchases_model->update($invoice_data, $id);
                if ($success) {
        ...
        if ($id == '') {
            ...
            $title = _l('edit', _l('purchase')) . ' - ' . $invoice->reference_no;
        }
        ...
        $data['title']     = $title;
        $data['bodyclass'] = 'purchase';
        $this->load->view('admin/purchases/invoice', $data);
    }
...
//        $data['payments']                   = $this->payments_model->get_invoice_payments($id);
        $data['activity']                   = $this->purchases_model->get_invoice_activity($id);
        $data['totalNotes']                 = total_rows('tblnotes', ['rel_id' => $id, 'rel_type' => 'purchase']);
//        $data['invoice_recurring_invoices'] = $this->purchases_model->get_invoice_recurring_invoices($id);
...
//		$data['invoices_delivery_modules']  = $this->delivery_module_model->get_delivery_modules_by_invoice_id($id);
        $data['invoice'] = $invoice;
        $this->load->view('admin/purchases/invoice_preview_template', $data);
    }
    ...
    public function apply_credits($invoice_id)
    {
        ...
        redirect(admin_url('purchases/list_purchases/' . $invoice_id));
    }
    ...
    public function record_invoice_payment_ajax($id)
    {
        ...
        $data['invoice']  = $invoice  = $this->purchases_model->get($id);
        $data['payments'] = $this->payments_model->get_invoice_payments($id);
    ...
    public function send_to_email($id)
    {
        ...
        try {
            $success = $this->purchases_model->send_invoice_to_client($id, '', $this->input->post('attach_pdf'), $this->input->post('cc'));
        } catch (Exception $e) {
    ...
    public function delete($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('purchases/list_purchases'));
        }
        ...
        if (strpos($_SERVER['HTTP_REFERER'], 'list_purchases') !== false) {
            redirect(admin_url('purchases/list_purchases'));
        } else {
    ...
    public function stock_in_delete($id)
    {
        ...
        $success = $this->purchases_model->stock_in_delete($id);
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->purchases_model->delete_attachment($id);
        } else {
    ...
    public function send_overdue_notice($id)
    {
        ...
        $send = $this->purchases_model->send_invoice_overdue_notice($id);
        if ($send) {
    ...
    public function mark_as_sent($id)
    {
        ...
        $success = $this->purchases_model->set_invoice_sent($id, true);
        if ($success) {
    ...
    public function get_due_date()
    {
	 ...
	 public function delivery_details($invoice_id = 0){
		 ...
		 $invoice			= $this->purchases_model->get($invoice_id);
