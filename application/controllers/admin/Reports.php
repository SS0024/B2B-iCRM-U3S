...
class Reports extends Admin_controller
{
    ...
    public function inquiries()
    {
        ...
        if (is_using_multiple_currencies() || is_using_multiple_currencies('tblcreditnotes') || is_using_multiple_currencies('tblinquiries') || is_using_multiple_currencies('tblproposals')) {
            $this->load->model('currencies_model');
            $data['currencies'] = $this->currencies_model->get();
        }
        $this->load->model('invoices_model');
        ...
        $data['credit_notes_statuses'] = $this->credit_notes_model->get_statuses();
        $data['invoice_statuses'] = $this->invoices_model->get_statuses();
        $data['estimate_statuses'] = $this->inquiries_model->get_statuses();
        $data['lead_statuses'] = $this->leads_model->get_status();
        $data['lead_sorces'] = $this->leads_model->get_source();
        $data['payments_years'] = $this->reports_model->get_distinct_payments_years();
        $data['estimates_sale_agents'] = $this->inquiries_model->get_sale_agents();
        ...
        $data['title'] = _l('sales_reports');
        $this->load->view('admin/reports/inquiries', $data);
    }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
				...
				if($aRow['devide_gst'] == 0){ 				
					$row[] = format_money($aRow['total_tax'], $currency_symbol);
					$footer_data['igst'] += $aRow['total_tax'];
				}else{
					$row[] = format_money(0, $currency_symbol);
				...
				if($aRow['devide_gst'] == 1 &&  $aRow['total_tax']!= 0){ 
					...
					$row[] = format_money($cgst, $currency_symbol);
					$footer_data['cgst'] += $cgst;
					
					$sgst = $aRow['total_tax']/2;
					$row[] = format_money($sgst, $currency_symbol);
					$footer_data['sgst'] += $sgst;
				...
				// $row[] = format_money($aRow['transportation'], $currency_symbol);
                // $footer_data['transportation'] += $aRow['transportation'];
				
				$row[] = format_money($aRow['servicecharge'], $currency_symbol);
                $footer_data['servicecharge'] += $aRow['servicecharge'];
				
				
				$row[] = format_money($aRow['packing_and_forwarding'], $currency_symbol);
                $footer_data['packing_and_forwarding'] += $aRow['packing_and_forwarding'];
    ...
    public function items()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($v && strpos($v->version, '5.7') !== false) {
                $aColumns = [
                        'ANY_VALUE(tblitems_in.description) as description',
                        'ANY_VALUE(tblitems_in.long_description) as long_description',
                        'ANY_VALUE(tblitems_brands.name) as brand_name',
                        'ANY_VALUE((SUM(tblitems_in.qty))) as quantity_sold',
                        'ANY_VALUE(SUM(tblitems_in.rate*tblitems_in.qty)) as rate',
                        'ANY_VALUE(AVG(tblitems_in.rate*tblitems_in.qty)) as avg_price',
                    ];
            } else {
                $aColumns = [
                        ...
                        'SUM(tblitems_in.rate*tblitems_in.qty) as rate',
                        'AVG(tblitems_in.rate*tblitems_in.qty) as avg_price',
                    ];
            }

            $sIndexColumn = 'id';
            $sTable       = 'tblitems_in';
            $join         = ['JOIN tblinvoices ON tblinvoices.id = tblitems_in.rel_id','LEFT JOIN tblitems ON tblitems_in.description = tblitems.description','LEFT JOIN tblitems_brands ON tblitems_brands.id = tblitems.brand'];
    ...
    public function item_report($id)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->app->get_table_data('item_report',[
                'id'=>$id,
                'type'=>$type
            ]);
        }
        $data['title'] = _l('Item Report');
    ...
    public function items_transaction()
    {
        if ($this->input->is_ajax_request()) {
            ...
            // 5.6 mysql version don't have the ANY_VALUE function implemented.
            $pp = "(SELECT 
	pi.product_id, SUM(CASE WHEN pi.purchase_id IS NOT NULL THEN pi.quantity ELSE pi.quantity END) purchasedQty, 
	SUM(pi.quantity_balance) balacneQty,
	MAX(pi.real_unit_cost) avgPurchaseCOst,
	SUM(pi.real_unit_cost * pi.quantity_balance) balacneValue,
	 SUM((CASE WHEN pi.purchase_id IS NOT NULL THEN (pi.real_unit_cost * pi.quantity) ELSE (pi.real_unit_cost * pi.quantity) END)) totalPurchase
FROM tblpurchaseitems pi
LEFT JOIN tblpurchases p ON p.id = pi.purchase_id GROUP BY pi.product_id) PCosts";
            $sp = "(SELECT si.description, SUM(si.qty) soldQty, SUM((si.rate * si.qty)) totalSale
FROM tblinvoices s
JOIN tblitems_in si ON (si.rel_type = 'invoice' AND si.rel_id = s.id) 
GROUP BY si.description) PSales";
            if ($v && strpos($v->version, '5.7') !== false) {
                $aColumns = [
                ...
                $aColumns = [
                        ...
                        'tblitems.long_description as long_description',
                        'CONCAT("(",COALESCE( PCosts.purchasedQty, 0 ), ")", COALESCE( PCosts.totalPurchase, 0 )) purchased',
                        'CONCAT("(",COALESCE( PSales.soldQty, 0 ), ")" ,COALESCE( PSales.totalSale, 0 )) sold',
                        '(COALESCE( PSales.totalSale, 0 ) - COALESCE( PCosts.totalPurchase, 0 )) Profit',
                        'CONCAT("(",COALESCE( (PCosts.purchasedQty- PSales.soldQty), 0 ), ")", COALESCE( (PCosts.avgPurchaseCOst*(PCosts.purchasedQty- PSales.soldQty)), 0 )) balance',
                    ];
            }

            $sIndexColumn = 'id';
            $sTable       = 'tblitems';
            $join         = ['JOIN '.$pp.' ON tblitems.id = PCosts.product_id','JOIN '.$sp.' ON tblitems.description = PSales.description'];
            $where = array();
//            $where = ['AND rel_type="invoice"', 'AND status != 5', 'AND status=2'];
            ...
            foreach ($rResult as $aRow) {
                $row = [];
                $row[] = $_data = '<a href="' . admin_url('reports/item_report/' . $aRow['id']) . '">' . $aRow['description'] . '</a>';;
                $row[] = $aRow['long_description'];
                $row[] = $aRow['purchased'];
                $row[] = $aRow['sold'];
                $row[] = $aRow['Profit'];
    ...
    public function inquiries_report()
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            $this->load->model('inquiries_model');

            $estimateTaxes = $this->distinct_taxes('inquiry');
            $totalTaxesColumns = count($estimateTaxes);
            ...
            foreach ($estimatesTaxesSelect as $key => $tax) {
                array_splice($select, 9, 0, '(
                    ...
                    FROM tblitems_in
                    INNER JOIN tblitemstax ON tblitemstax.itemid=tblitems_in.id
                    WHERE tblitems_in.rel_type="inquiry" AND taxname="' . $tax['taxname'] . '" AND taxrate="' . $tax['taxrate'] . '" AND tblitems_in.rel_id=tblinquiries.id) as total_tax_single_' . $key);
            }
            ...
            if ($this->input->post('lead_status')) {
                $agents = $this->input->post('lead_status');
                $_agents = [];
                if (is_array($agents)) {
                    foreach ($agents as $agent) {
                        if ($agent != '') {
                ...
                if (count($_agents) > 0) {
                    array_push($where, 'AND (lead_status Like '. implode(' OR lead_status LIKE ',$_agents).')');
                }
            }
            if ($this->input->post('lead_source')) {
                $agents = $this->input->post('lead_source');
                $_agents = [];
                if (is_array($agents)) {
                    foreach ($agents as $agent) {
                        if ($agent != '') {
                ...
                if (count($_agents) > 0) {
                    array_push($where, 'AND (lead_source Like '. implode(' OR lead_source LIKE ',$_agents).')');
                }
            }
            if ($this->input->post('sale_agent_estimates')) {
                $agents = $this->input->post('sale_agent_estimates');
            ...
            $sTable = 'tblinquiries';
            $join = [
                'LEFT JOIN tblclients ON tblclients.userid = tblinquiries.rel_id',
            ];
            ...
            $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
                ...
                'rel_id',
                'tblinquiries.id',
                'tblinquiries.lead_status as lead_status',
                'tblinquiries.lead_source as lead_source',
                'tblinquiries.assigned as sale_agent',
                'discount_percent',
            ]);
            ...
            foreach ($rResult as $aRow) {
                ...
                $this->db->where('staffid', $aRow['sale_agent']);
                $this->db->order_by('staffid', 'asc');
                $saleAgent = $this->db->get()->row();

                $row[] = '<a href="' . admin_url('inquiries/list_inquiries/' . $aRow['id']) . '" target="_blank"> INQ-' . str_pad($aRow['id'], get_option('number_padding_prefixes'), '0', STR_PAD_LEFT) . '</a>';
                ...
                $contactPerson = [];
                $divisions = [];
                $divConArray = get_div_cons_by_type('inquiry', $aRow['id']);
                ...
                $estimateItem = get_items_by_type('inquiry', $aRow['id'])[0];
                $this->db->select('*');
                $this->db->from('tblitems');
                $this->db->where('description', $estimateItem['description']);
                $recdata = $this->db->get()->row();

                $this->db->select('*');
                $this->db->from('tblitems_groups');
                $this->db->where('id', $recdata->group_id);
                $groupdata = $this->db->get()->row();

                $this->db->select('*');
                $this->db->from('tblitems_brands');
                $this->db->where('id', $recdata->brand);
                $branddata = $this->db->get()->row();
                $row[] = $branddata->name;
    ...
    public function inquiry_export_report()
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            $this->load->model('inquiries_model');
            ...
            $select = [
                '(SELECT CONCAT(firstname, " ", lastname) FROM tblstaff WHERE staffid = tblinquiries.assigned) as assigned_to',
                 get_sql_select_client_company(),
                'tblinquiries.id as id',
                'date',
                'status',
                'lead_status',
                'lead_source',
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = $groupdata->name;
                $row[] = format_money($aRow['total'], $currency_symbol);
                $row[] = $aRow['invoice_id'] ? 'true' : 'false';
                $row[] = $aRow['is_rfr'] ? 'true' : 'false';
    ...
    public function product_inquiries_report()
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            $this->load->model('inquiries_model');
            ...
            foreach ($rResult as $aRow) {
                ...
                $inquiryNo = '<a href="' . admin_url('inquiries/list_inquiries/' . $aRow['id']) . '" target="_blank"> INQ-' . str_pad($aRow['id'], get_option('number_padding_prefixes'), '0', STR_PAD_LEFT) . '</a>';

                $inquiryDate = _d($aRow['date']);

//                $row[] = format_estimate_status($aRow['status']);

                $leadStatus = $aRow['lead_status'];
                $leadSource = $aRow['lead_source'];
                ...
                $contact = implode(', ', $contactPerson);

                if (empty($aRow['deleted_customer_name'])) {
                    $customer = '<a href="' . admin_url('clients/client/' . $aRow['userid']) . '" target="_blank">' . $aRow['company'] . '</a>';
                } else {
                    $customer = $aRow['deleted_customer_name'];
                }

                $divisions = implode(', ', array_values(array_unique($divisions)));
                $estimateItems = get_items_by_type('inquiry', $aRow['id']);
                $adminNote = $aRow['adminnote'];
                $assigned = $saleAgent->firstname . ' ' . $saleAgent->lastname;
                foreach ($estimateItems as $estimateItem){
                    ...
                    $row[]=$recdata->long_description;
                    $row[]=$recdata->warranty;
                    $row[]= _d($recdata->upto);
                    $row[]= $estimateItem['qty'];
                    $row[]= $estimateItem['item_amount'];
                    $row[]=$estimateItem['item_discount'];
                    $row[]= $estimateItem['rate'];
                    $row[]= $adminNote;
                    $row[]= $assigned;
                    $output['aaData'][] = $row;
                }
    ...
    public function product_status_quated_item_report()
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            $this->load->model('inquiries_model');
            ...
            foreach ($rResult as $aRow) {
                ...
                $assignedName = $aRow['assigned_to'];
                if (empty($aRow['deleted_customer_name'])) {
                    $customerName = '<a href="' . admin_url('clients/client/' . $aRow['userid']) . '" target="_blank">' . $aRow['company'] . '</a>';
                } else {
                    $customerName = $aRow['deleted_customer_name'];
                }
                $inquiryNo = '<a href="' . admin_url('inquiries/list_inquiries/' . $aRow['id']) . '" target="_blank"> INQ-' . str_pad($aRow['id'], get_option('number_padding_prefixes'), '0', STR_PAD_LEFT) . '</a>';
                $inquiryDate = _d($aRow['date']);
                $inquiryStatus = format_proposal_status($aRow['status']);
                $leadStatus = $aRow['lead_status'];
                $leadSource = $aRow['lead_source'];
                $adminNote = $aRow['adminnote'];
                $referenceNo = $aRow['reference_no'];
                $contactPerson = [];
                $divisions = [];
                $divConArray = get_div_cons_by_type('inquiry', $aRow['id']);
                foreach ($divConArray as $con_div)
                ...
                $contact = implode(', ', $contactPerson);
                $divisions = implode(', ', array_values(array_unique($divisions)));
                $totalVal = format_money($aRow['total'], $currency_symbol);
                $poStatus = $aRow['invoice_id'] ? 'true' : 'false';
                $RFRStatus = $aRow['is_rfr'] ? 'true' : 'false';
    ...
    public function rfr_export_report()
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            $this->load->model('inquiries_model');
            ...
            $where = [];
            $custom_date_select = $this->get_where_report_period('created_at');
            if ($custom_date_select != '') {
                array_push($where, $custom_date_select);
            }
            ...
            $sTable = 'tblrfr';
            $join = [
                'LEFT JOIN tblstaff ON tblstaff.staffid = tblrfr.staffid',
                'LEFT JOIN tblitems ON tblitems.id = tblrfr.item_id',
