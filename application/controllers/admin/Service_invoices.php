...
class Service_invoices extends Admin_controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('service_invoices_model');
    ...
    {
        if (!has_permission('service_invoices', '', 'view')
            && !has_permission('service_invoices', '', 'view_own')
            ) {
            access_denied('service_invoices');
        }
        ...
        $data['invoiceid']            		= $id;
        $data['title']               		= _l('Service Invoices');
        $data['invoices_years']       		= $this->service_invoices_model->get_invoices_years();
        $data['invoices_sale_agents'] 		= $this->service_invoices_model->get_sale_agents();
        $data['invoices_statuses']    		= $this->service_invoices_model->get_statuses();
        $data['bodyclass']            		= 'invoices-total-manual';
        $this->load->view('admin/service_invoices/manage', $data);
    }
    ...
    public function recurring($id = '')
    {
        ...
        $data['invoices_years']       = $this->service_invoices_model->get_invoices_years();
        $data['invoices_sale_agents'] = $this->service_invoices_model->get_sale_agents();
        $this->load->view('admin/service_invoices/recurring/list', $data);
    }
    ...
    public function table($clientid = '')
    {
        ...
        $this->app->get_table_data(($this->input->get('recurring') ? 'recurring_invoices' : 'service_invoices'), [
            'clientid' => $clientid,
            'data'     => $data,
        ]);
    }
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($current_invoice != '') {
                ...
                $current_invoice_status = $this->db->get('tblserviceinvoices')->row()->status;
            }

            $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != 5) ? $this->service_invoices_model->check_for_merge_invoice($customer_id, $current_invoice) : [];

            $data['merge_info'] = $this->load->view('admin/service_invoices/merge_invoice', $_data, true);

            $this->load->model('currencies_model');

            $__data['expenses_to_bill'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != 5) ? $this->service_invoices_model->get_expenses_to_bill($customer_id) : [];

            $data['expenses_bill_info'] = $this->load->view('admin/service_invoices/bill_expenses', $__data, true);
    ...
    public function update_number_settings($id)
    {
        ...
        if (has_permission('service_invoices', '', 'edit')) {
            $affected_rows = 0;

            $this->db->where('id', $id);
            $this->db->update('tblserviceinvoices', [
    ...
    public function validate_invoice_number()
    {
        ...
        if (total_rows('tblserviceinvoices', [
    ...
    {
        if ($this->input->post()) {
            $this->misc_model->add_note($this->input->post(), 'service_invoice', $rel_id);
            echo $rel_id;
        }
    }
    ...
    {
        $data['notes'] = $this->misc_model->get_notes($id, 'service_invoice');
    ...
    public function pause_overdue_reminders($id)
    {
        ...
        redirect(admin_url('service_invoices/list_invoices/' . $id));
    }
    ...
    {
        if (!has_permission('service_invoices', '', 'edit') && !has_permission('service_invoices', '', 'create')) {
            access_denied('service_invoices');
        }

        $success = $this->service_invoices_model->mark_as_cancelled($id);
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $success = $this->service_invoices_model->unmark_as_cancelled($id);
        if ($success) {
    ...
    {
        if (!$id) {
            redirect(admin_url('service_invoices'));
        }
        if (!has_permission('service_invoices', '', 'create')) {
            access_denied('service_invoices');
        }
        $new_id = $this->service_invoices_model->copy($id);
        if ($new_id) {
            set_alert('success', _l('invoice_copy_success'));
            redirect(admin_url('service_invoices/invoice/' . $new_id));
        } else {
            set_alert('success', _l('invoice_copy_fail'));
        }
        redirect(admin_url('service_invoices/invoice/' . $id));
    }

    public function get_merge_data($id)
    {
        $invoice = $this->service_invoices_model->get($id);
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $id = $this->service_invoices_model->add($invoice_data);
                if(isset($task_id)){
                    $this->db->where('id', $task_id);
                    $this->db->update('tblstafftasks', ['service_invoice_id' => $id]);
                }
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('Service invoice')));
                    redirect(admin_url('service_invoices/list_invoices/' . $id));
                }
            } else {
                if (!has_permission('service_invoices', '', 'edit')) {
                ...
                $success = $this->service_invoices_model->update($invoice_data, $id);
                if ($success) {
                    set_alert('success', _l('updated_successfully', _l('Service invoice')));
                }
                redirect(admin_url('service_invoices/list_invoices/' . $id));
            }
        }
        ...
        if ($id == '') {
            $title                  = _l('Create new service invoice');
            $data['billable_tasks'] = [];
        } else {
            $invoice = $this->service_invoices_model->get($id);

            if (!$invoice) {
                blank_page('Service '._l('invoice_not_found'));
            }

            $data['invoices_to_merge'] = $this->service_invoices_model->check_for_merge_invoice($invoice->clientid, $invoice->id);
            $data['expenses_to_bill']  = $this->service_invoices_model->get_expenses_to_bill($invoice->clientid);

            $data['invoice']        = $invoice;
            $data['edit']           = true;
            $data['billable_tasks'] = $this->tasks_model->get_billable_tasks($invoice->clientid, !empty($invoice->project_id) ? $invoice->project_id : '');

            $title = _l('edit', 'service '._l('invoice_lowercase')) . ' - ' . format_service_invoice_number($invoice->id);
        }
        ...
        if($this->input->get('task_id')){
            $this->db->select('*');
            $this->db->join('tblcontracts', 'tblcontracts.id = tblstafftasks.rel_id and tblstafftasks.rel_type = "contract"');
        ...
        $this->load->view('admin/service_invoices/invoice', $data);
    }
    ...
    {
        if (!has_permission('service_invoices', '', 'view') && !has_permission('service_invoices', '', 'view_own') ) {
        ...
        $data['invoices_to_merge'] = $this->service_invoices_model->check_for_merge_invoice($invoice->clientid, $id);
        ...
        $data['activity']                   = $this->service_invoices_model->get_invoice_activity($id);
        $data['totalNotes']                 = total_rows('tblnotes', ['rel_id' => $id, 'rel_type' => 'invoice']);
        $data['invoice_recurring_invoices'] = $this->service_invoices_model->get_invoice_recurring_invoices($id);
        ...
        $data['invoice'] = $invoice;
        $this->load->view('admin/service_invoices/invoice_preview_template', $data);
    }
    ...
    public function apply_credits($invoice_id)
    {
        ...
        redirect(admin_url('service_invoices/list_invoices/' . $invoice_id));
    }
    ...
    public function record_invoice_payment_ajax($id)
    {
        ...
        $data['invoice']  = $invoice  = $this->service_invoices_model->get($id);
        $data['payments'] = $this->invoice_payments_model->get_invoice_payments($id);
        $this->load->view('admin/service_invoices/record_payment_template', $data);
    }
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url('service_invoices/list_invoices/' . $this->input->post('invoiceid')));
    ...
    {
        if (!has_permission('service_invoices', '', 'view') && !has_permission('service_invoices', '', 'view_own') && $canView == false) {
            access_denied('service_invoices');
        }

        try {
            $success = $this->service_invoices_model->send_invoice_to_client($id, '', $this->input->post('attach_pdf'), $this->input->post('cc'));
        } catch (Exception $e) {
    ...
    {
        if (!has_permission('service_invoices', '', 'delete')) {
            access_denied('service_invoices');
        }
        if (!$id) {
            redirect(admin_url('service_invoices/list_invoices'));
        }
        $success = $this->service_invoices_model->delete($id);
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->service_invoices_model->delete_attachment($id);
        } else {
    ...
    public function send_overdue_notice($id)
    {
        ...
        $send = $this->service_invoices_model->send_invoice_overdue_notice($id);
        if ($send) {
    ...
    public function pdf($id)
    {
        ...
        $invoice_number = format_service_invoice_number($invoice->id);

        try {
            $pdf = service_invoice_pdf($invoice);
        } catch (Exception $e) {
            $message = $e->getMessage();
            echo $message;
            if (strpos($message, 'Unable to get the size of the image') !== false) {
    ...
    public function mark_as_sent($id)
    {
        ...
        $success = $this->service_invoices_model->set_invoice_sent($id, true);
        if ($success) {
    ...
    public function get_due_date()
    {
	 ...
	 public function delivery_details($invoice_id = 0){
		 ...
		 $invoice			= $this->service_invoices_model->get($invoice_id);
			...
			if ($this->input->post()) {
				...
				// }else{
					...
					if ($id) {
						...
						echo json_encode([
							'url'       => admin_url('service_invoices/list_invoices#/' . $invoice_id),
		...
		// $data['delivery_detail']   = $delivery_detail; 
		 ...
		 $this->load->view('admin/service_invoices/delivery_details', $data);
	 }
	 ...
	 public function deliveries_old($invoice_id = 0){
		...
		$this->load->view('admin/service_invoices/deliveries', $data);
	 }
    ...
    {      
        ...
        $this->load->view('admin/service_invoices/manage_deliveries', $data);
    }
	...
	public function delete_delivery_module_attachment($id,$invoice_id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
           ...
           redirect(admin_url('service_invoices/delivery_details/' . $invoice_id));
        } else {
    ...
    public function get_latest_number(){
        $next_estimate_number = get_option('next_service_invoice_number');
