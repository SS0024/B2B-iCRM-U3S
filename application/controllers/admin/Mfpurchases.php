...
class Mfpurchases extends Admin_controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('mfpurchases_model');
    ...
    {
        $this->list_mfpurchases($id);
    }

    /* List all invoices datatables */
    public function list_mfpurchases($id = '')
    {
        if (!has_permission('mfpurchases', '', 'view')
            && !has_permission('mfpurchases', '', 'view_own')) {
            access_denied('mfpurchases');
        }
        ...
        $data['invoiceid']            		= $id;
        $data['title']               		= _l('Purchases');
        $data['invoices_years']       		= $this->mfpurchases_model->get_invoices_years();
        $data['invoices_sale_agents'] 		= $this->mfpurchases_model->get_sale_agents();
        $data['invoices_statuses']    		= $this->mfpurchases_model->get_statuses();
        $data['bodyclass']            		= 'purchases-total-manual';
        $this->load->view('admin/mfpurchases/manage', $data);
    }

    /* List all invoices datatables */
    public function stock_in_list($id = '')
    {
        if (!has_permission('stock_in', '', 'view')
            && !has_permission('stock_in', '', 'view_own')) {
            access_denied('stock_in');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('stock_list');
        }
        $data['title'] = _l('Stock in list');
        $this->load->view('admin/purchases/stock_list', $data);
    }
    ...
    public function table($clientid = '')
    {
        ...
        $this->app->get_table_data('mfpurchases', [
            'clientid' => $clientid,
            'data'     => $data,
        ]);
    }
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != 5) ? $this->mfpurchases_model->check_for_merge_invoice($customer_id, $current_invoice) : [];

            $data['merge_info'] = $this->load->view('admin/invoices/merge_invoice', $_data, true);

            $this->load->model('currencies_model');

            $__data['expenses_to_bill'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != 5) ? $this->mfpurchases_model->get_expenses_to_bill($customer_id) : [];
    ...
    public function stock_in($purchase_id)
    {
        if (!has_permission('stock_in', '', 'create')) {
            access_denied('stock_in');
        }
        $invoice = $this->mfpurchases_model->get($purchase_id);
        if ($this->input->post()) {
            $products = [];
            ...
            $newDate = $products;
            $success = $this->mfpurchases_model->updateQuantityBalance($newDate, $invoice->warehouse_id);
            $this->db->where('id', $purchase_id);
            $this->db->update('tblpurchases', ['is_stock_in'=>2]);
            set_alert('success', 'Stock in successfully.');
            redirect(admin_url('purchases/list_mfpurchases/'.$invoice->id));

        }
        $data['adjustment'] = $invoice;
        $this->load->view('admin/purchases/stock_in', $data);
    }
    ...
    public function pause_overdue_reminders($id)
    {
        ...
        redirect(admin_url('invoices/list_mfpurchases/' . $id));
    }
    ...
    {
        if (!has_permission('purchases', '', 'edit') && !has_permission('purchases', '', 'create')) {
            access_denied('purchases');
        }

        $success = $this->mfpurchases_model->mark_as_cancelled($id);

        if ($success) {
            set_alert('success', _l('invoice_marked_as_cancelled_successfully'));
        }

        redirect(admin_url('purchases/list_mfpurchases/' . $id));
    }
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $success = $this->mfpurchases_model->unmark_as_cancelled($id);
        if ($success) {
    ...
    public function copy($id)
    {
        ...
        $new_id = $this->mfpurchases_model->copy($id);
        if ($new_id) {
    ...
    {
        $invoice = $this->mfpurchases_model->get($id);
    ...
    public function mfpurchase($id = '')
    {
        if ($this->input->post()) {
            $invoice_data = $this->input->post();

            if ($id == '') {
                if (!has_permission('mfpurchases', '', 'create')) {
                    access_denied('mfpurchases');
                }
				//print_r($invoice_data);die;
                $id = $this->mfpurchases_model->add($invoice_data);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('Purchases')));
                    redirect(admin_url('mfpurchases/list_mfpurchases/' . $id));
                }
            } else {
                if (!has_permission('mfpurchases', '', 'edit')) {
                    access_denied('mfpurchases');
                }
                $success = $this->mfpurchases_model->update($invoice_data, $id);
                if ($success) {
                    set_alert('success', _l('updated_successfully', _l('Purchases')));
                }
                redirect(admin_url('mfpurchases/list_mfpurchases/' . $id));
            }
        }
        ...
        if ($id == '') {
            $title                  = _l('Create new MFPurchase');
            $data['billable_tasks'] = [];
        } else {
            $invoice = $this->mfpurchases_model->get($id);

            if (!$invoice) {
                blank_page(_l('Purchase not found'));
            }
            ...
            $title = _l('edit', _l('mfpurchase')) . ' - ' . $invoice->reference_no;
        }
        ...
        $data['title']     = $title;
        $data['bodyclass'] = 'mfpurchase';
        $this->load->view('admin/mfpurchases/mfinvoice', $data);
    }


    /* Add Supplier Price or update existing */
    public function supplier_price($id = '')
    {
        if ($this->input->post()) {
            $Supplier_price_data = $this->input->post();
                if (!has_permission('mfpurchases', '', 'edit')) {
                    access_denied('mfpurchases');
                }

                foreach ($Supplier_price_data['supplier_id'] as  $supplier_price_data){

                    $supplier_id = $supplier_price_data;
                    $supplier_item_price = $Supplier_price_data[$supplier_price_data];

                    $success = $this->mfpurchases_model->update_supplier_price($supplier_item_price, $supplier_id);

                }

                if ($success == 1) {
                    set_alert('success', _l('updated_successfully', _l('Supplier Price')));
                }
                redirect(admin_url('mfpurchases/list_mfpurchases/' . $id));
        ...
        if ($id == '') {
            ...
            blank_page(_l('Please Select Mfpurcheses Invoice when you want Add or Edit Supplier'));
        } else {
            $suppliers_data = $this->mfpurchases_model->suppliers_data_get();
            $items = $this->mfpurchases_model->item_get($id);
            $suppliers = $this->mfpurchases_model->suppliers_get($id);

            if (!$items) {
                blank_page(_l('MFPurchase Items not found'));
            }
            $data['items']  = $items;
            $data['suppliers']  = $suppliers;
            $data['suppliers_data']  = $suppliers_data;

            $title = _l('edit', _l('Supplier Price')) . ' - ' . $id;


            $data['title']     = $title;
            $this->load->view('admin/mfpurchases/supplier_price', $data);
        }
    ...
    {
        if (!has_permission('mfpurchases', '', 'view') && !has_permission('mfpurchases', '', 'view_own')) {
            echo _l('access_denied');
            die;
        }

        if (!$id) {
            die(_l('Purchase not found'));
        }

        $invoice = $this->mfpurchases_model->get($id);
        if (!$invoice) {
            echo _l('Purchase not found');
            die;
        }

        $invoice->date    = _dt($invoice->date);
        ...
        $data['activity']                   = $this->mfpurchases_model->get_invoice_activity($id);
        $data['totalNotes']                 = total_rows('tblnotes', ['rel_id' => $id, 'rel_type' => 'purchase']);
        $data['invoice'] = $invoice;
        $this->load->view('admin/mfpurchases/invoice_preview_template', $data);
    }
    ...
    public function apply_credits($invoice_id)
    {
        ...
        redirect(admin_url('purchases/list_mfpurchases/' . $invoice_id));
    }
    ...
    public function record_invoice_payment_ajax($id)
    {
        ...
        $data['invoice']  = $invoice  = $this->mfpurchases_model->get($id);
        $data['payments'] = $this->mfpayments_model->get_invoice_payments($id);
    ...
    public function send_to_email($id)
    {
        ...
        try {
            $success = $this->mfpurchases_model->send_invoice_to_client($id, '', $this->input->post('attach_pdf'), $this->input->post('cc'));
        } catch (Exception $e) {
    ...
    public function delete($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('mfpurchases/list_mfpurchases'));
        }
        $success = $this->mfpurchases_model->delete($id);

        if ($success) {
            set_alert('success', _l('deleted', _l('Purchase')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('purchase')));
        }
        if (strpos($_SERVER['HTTP_REFERER'], 'list_mfpurchases') !== false) {
            redirect(admin_url('purchases/list_mfpurchases'));
        } else {
    ...
    public function stock_in_delete($id)
    {
        ...
        $success = $this->mfpurchases_model->stock_in_delete($id);

        if ($success) {
            set_alert('success', _l('deleted', _l('Stock In')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('stock in')));
        }
        if (strpos($_SERVER['HTTP_REFERER'], 'list_mfpurchases') !== false) {
            redirect(admin_url('purchases/stock_in_list'));
        } else {
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->mfpurchases_model->delete_attachment($id);
        } else {
    ...
    public function send_overdue_notice($id)
    {
        ...
        $send = $this->mfpurchases_model->send_invoice_overdue_notice($id);
        if ($send) {
    ...
    public function stock_in_pdf($id)
    {
        if (!$id) {
            redirect(admin_url('purchases/stock_in_list'));
        }
        ...
        $invoice_number = format_purchase_number($invoice->id);

        try {
            $pdf = stock_in_pdf($invoice);
        } catch (Exception $e) {
            $message = $e->getMessage();
            echo $message;
            if (strpos($message, 'Unable to get the size of the image') !== false) {
    ...
    public function mark_as_sent($id)
    {
        ...
        $success = $this->mfpurchases_model->set_invoice_sent($id, true);
        if ($success) {
    ...
    public function get_due_date()
    {
	 ...
	 public function delivery_details($invoice_id = 0){
		 ...
		 $invoice			= $this->mfpurchases_model->get($invoice_id);
