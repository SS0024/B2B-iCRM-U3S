...
class Leads extends Admin_controller
{
    ...
    private function _get_lead_data($id = '')
    {
        ...
        $data['statuses'] = $this->leads_model->get_status();
        $data['sources']  = $this->leads_model->get_source();
		$data['lead_profile_attachment']	=	$this->leads_model->get_lead_profile_attachment($lead->id);
	...
	public function add_lead_profile_attachment($lead_id = 0){
		if (!is_staff_member() || !$this->leads_model->staff_can_access_lead($lead_id)) {
            $this->access_denied_ajax();
        }

        handle_lead_attachments($lead_id,'file',false,"lead_profile");
	}
	
	
	public function get_lead_profile_attachment($lead_id = 0){
		$data = $this->leads_model->get_lead_profile_attachment($lead_id);
		$result = '';
		if(!empty($data)){
			$result .= '<div class="display-block lead-attachment-wrapper" id="lead-profile-attachment-wrapper">
					 <div class="col-md-10">';
					 
					 $path = get_upload_path_by_type('lead').'/'.$data->rel_id.'/'.$data->file_name;
					 
					 $href_url = site_url('download/file/lead_attachment/'.$data->id.'/'.$data->file_name);
						
					 $img_url = site_url('download/preview_image?path='.protected_file_url_by_path($path,true).'&type='.$data->filetype);
					
			$result .= 	'<div class="preview-image">
						 <a href="'.$href_url.'" target="_blank" data-lightbox="task-attachment" class="">
							<img src="'.$img_url.'" class="img img-responsive" width="200px" height="150px">
						 </a>
						</div>					 
					 </div>';
					 if($data->attachment_added_from == get_staff_user_id() || is_admin()){
							$result .= 	'<div class="col-md-2 text-right">
							<a href="#" class="text-danger _delete" onclick="delete_lead_attachment(this,' .$data->id.','.$data->rel_id.'); return false;"><i class="fa fa fa-times"></i></a>
						 </div>';
					 
					 }
					 
					 $result .= '<div class="clearfix"></div><hr></div>';
		}
