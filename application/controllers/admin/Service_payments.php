...
class Service_payments extends Admin_controller
{
    ...
    {
        if (!has_permission('payments', '', 'view') && !has_permission('service_invoices', '', 'view_own')) {
            access_denied('payments');
        }

        $data['title'] = _l('payments');
        $this->load->view('admin/service_payments/manage', $data);
    }
    ...
    public function table($clientid = '')
    {
        ...
        $this->app->get_table_data('service_payments', [
            'clientid' => $clientid,
        ]);
    }
    ...
    public function payment($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            $success = $this->invoice_payments_model->update($this->input->post(), $id);
            if ($success) {
                set_alert('success', _l('updated_successfully', _l('payment')));
            }
            redirect(admin_url('service_payments/payment/' . $id));
        }
        $data['payment'] = $this->invoice_payments_model->get($id);
        if (!$data['payment']) {
            blank_page(_l('payment_not_exists'));
        }
        $this->load->model('service_invoices_model');
        $data['invoice'] = $this->service_invoices_model->get($data['payment']->invoiceid);
        ...
        $this->load->view('admin/service_payments/payment', $data);
    }
    ...
    public function pdf($id)
    {
        ...
        $payment = $this->invoice_payments_model->get($id);

        if (!has_permission('payments', '', 'view') && !has_permission('service_invoices', '', 'view_own') && !user_can_view_invoice($payment->invoiceid)) {
            access_denied('View Payment');
        }
        ...
        try {
            $paymentpdf = service_payment_pdf($payment);
        } catch (Exception $e) {
