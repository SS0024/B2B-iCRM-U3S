...
class Tasks extends Admin_controller
{
    ...
    public function list_tasks($id = '')
    {
        ...
        $data['customers'] = $this->clients_model->get_task_customers();
    ...
    /* List all tasks */
    public function list_service_tasks($id = '')
    {
        close_setup_menu();
        // If passed from url
        $data['custom_view'] = $this->input->get('custom_view') ? $this->input->get('custom_view') : '';
    ...
    public function task($id = '')
    {
        ...
        if ($this->input->get('hour')) {
            $data['hour'] = $this->input->get('hour');
        }
        if ($this->input->get('service_no')) {
            $data['service_no'] = $this->input->get('service_no');
        }
            ...
            if ($id == '') {
                ...
                if (isset($serviceNo) && $data['rel_type'] == 'contract'){
                    ...
                    if(count($services) > 0){
                        ...
                        $this->db->update('tblcontracts', $serviceData);
                    }
        ...
        if ($this->input->get('rel_type') == 'contract' && $this->input->get('rel_id')) {
            $project = $this->contracts_model->get($this->input->get('rel_id'));
            $data['service_details'] = $project;
            $this->db->select();
            $this->db->from('tblitems_in');
            $this->db->where('id', $project->item_id);
            $item=$this->db->get()->row();
            $data['service_details']->item = $item;
        }
        $data['id']    = $id;
    ...
    public function mark_as($status, $id)
    {
        if ($this->tasks_model->is_task_assignee(get_staff_user_id(), $id)
            ...
            $this->db->select('rel_type,rel_id,name,visible_to_client,status,billable,service_invoice_id');
            $this->db->where('id', $id);
            $task = $this->db->get('tblstafftasks')->row();

            if ($status == 5 && ($task->billable == 1 &&  $task->service_invoice_id==0 ) && ($task->rel_type == 'contract' && $task->rel_type == 'customer')) {
                echo json_encode([
                    'success'  => false,
                    'message'  => 'Please enter Service PI Details.',
                    'taskHtml' => $taskHtml,
                ]);exit();
            }
    ...
    public function changeRequsiteOptionValue($id,$value){
        if ($this->input->is_ajax_request()) {
            $this->db->where('id', $id);
            $this->db->update('tblstafftasks', ['is_requsite'=>$value]);
