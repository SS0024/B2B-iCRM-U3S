...
class Download extends CRM_Controller
{
    ...
    public function file($folder_indicator, $attachmentid = '')
    {
        ...
        // l_attachment_key is if request is coming from public form
        ...
        }elseif ($folder_indicator == 'delivery_modules') {
            if (!is_staff_logged_in()) {
                show_404();
            }
            $this->db->where('rel_id', $attachmentid);
            ...
            $file = $this->db->get('tblfiles')->row();
            $path = get_upload_path_by_type('delivery_modules') . $file->rel_id . '/' . $file->file_name;
        // l_attachment_key is if request is coming from public form
        }elseif ($folder_indicator == 'invoice_payment') {
            if (!is_staff_logged_in()) {
                show_404();
            }
            $this->db->where('rel_id', $attachmentid);
            $this->db->where('rel_type', 'invoice_payment');
            $file = $this->db->get('tblfiles')->row();
            $path = get_upload_path_by_type('invoice_payment') . $file->rel_id . '/' . $file->file_name;
        // l_attachment_key is if request is coming from public form
        }elseif ($folder_indicator == 'invoice_payment_temp') {
            if (!is_staff_logged_in()) {
                show_404();
            }
            $this->db->where('id', $attachmentid);
            $this->db->where('rel_type', 'invoice_payment');
            $file = $this->db->get('tblfilestemp')->row();
            $path = get_upload_path_by_type('invoice_payment_temp') . $file->file_name;
        // l_attachment_key is if request is coming from public form
        }
		else {
