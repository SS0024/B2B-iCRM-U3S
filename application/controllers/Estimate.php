...
class Estimate extends Clients_controller
{
    ...
    public function syncOldPONumber(){
        $q = $this->db->query("SELECT id,invoice_id FROM `tblinquiries` WHERE `invoice_id` IS NOT NULL ORDER BY `id` DESC");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $this->db->where('id', $row->invoice_id);
                $this->db->set('inquiry_id', $row->id, false);
                $this->db->update('tblpurchaseorder');
            }
        }
        $q = $this->db->query("SELECT id,invoiceid FROM `tblestimates` WHERE `invoiceid` IS NOT NULL ORDER BY `id` DESC");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $this->db->where('id', $row->invoiceid);
                $this->db->set('estimate_id', $row->id, false);
                $this->db->update('tblpurchaseorder');
            }
        }
    }
