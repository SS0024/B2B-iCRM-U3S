...
$info_right_column .= '<span style="font-weight:bold;font-size:27px;">' . _l('Delivery Challan') . '</span><br />';
...
$invoice_info .= '<br />' . _l('Delivery Challan Date:') . ' ' . _d($invoice->date) . '<br />';
...
$custom_fields_items = get_items_custom_fields_for_table_html($invoice->id, 'delivery_challan');
...
if($invoice->is_bulk==0){
    $tblhtml .= '<th align="right">' . _l('invoice_table_rate_heading') . '</th>';
}
if (get_option('show_tax_per_item') == 1) {
    $tblhtml .= '<th align="right">' . _l('invoice_table_tax_heading') . '</th>';
}

if($invoice->is_bulk==0){
    if($isDiscountDisplay) {
        $tblhtml .= '<th align="right">' . _l('invoice_table_discount_heading') . '</th>';
    }
    $tblhtml .= '<th align="right">' . _l('invoice_table_amount_heading') . '</th>';
}
$tblhtml .= '</tr>';

// Items
$tblhtml .= '<tbody>';
$items_data = get_table_items_and_taxes($invoice->items, 'delivery_challan', false,$estimate->is_bulk,$isDiscountDisplay);
...
//if ((int)$invoice->transportation != 0) {
    // $tbltotal .= '<tr>
    // <td align="right" width="85%"><strong>'._l('estimate_transportation').'</strong></td>
    // <td align="right" width="15%">' . format_money($invoice->transportation,$invoice->symbol) . '</td>
// </tr>';
//}

//if ((int)$invoice->packing_and_forwarding != 0) {
    $tbltotal .= '<tr>
...
//}
//if ((int)$invoice->servicecharge != 0) {
    $tbltotal .= '<tr>
...
//}
	...
	// if(isset($invoice->servicecharge) && $invoice->servicecharge > 0 && !empty($invoice->service_charge_tax_rate)){
		// $service_charge_tax	=	$invoice->servicecharge * $invoice->service_charge_tax_rate/100;
		 ...
		 // $tbltotal .= '<tr>
				// <td align="right" width="85%"><strong>' .'GST@'.$invoice->service_charge_tax_rate.'%' . ' (' . _format_number($invoice->service_charge_tax_rate) . '%)' . '</strong></td>
				// <td align="right" width="15%">' . format_money($service_charge_tax, $invoice->symbol) . '</td>
			// </tr>';
	// }
...
$items_data = get_table_items_and_taxes($invoice->items, 'delivery_challan');
