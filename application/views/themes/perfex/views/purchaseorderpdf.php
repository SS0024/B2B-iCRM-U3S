...
if( $_SESSION['xqut'] == 2)
{
	$info_right_column .= '<span style="font-weight:bold;font-size:27px;">'.'PERFORMA INVOICE'.'</span><br />';

}
else
{
	$info_right_column .= '<span style="font-weight:bold;font-size:27px;">'.'Purchase Order'.'</span><br />';
}
...
if(get_option('show_status_on_pdf_ei') == 1){
    $info_right_column .= '<br /><span style="color:rgb('.estimate_status_color_pdf($status).');text-transform:uppercase;">' . format_purchaseorder_status($status,'',false) . '</span>';
}
...
if($estimate->sale_agent != 0 && get_option('show_sale_agent_on_estimates') == 1){
    $estimate_info .= _l('sale_agent_string') . ': ' .  get_staff_full_name($estimate->sale_agent,true). '<br />';
}
...
foreach($pdf_custom_fields as $field){
    $value = get_custom_field_value($estimate->id,$field['id'],'purchaseorder');
    if($value == ''){continue;}
...
$custom_fields_items = get_items_custom_fields_for_table_html($estimate->id,'purchaseorder');
...
$items_data = get_table_items_and_taxes($estimate->items,'purchaseorder',true,$estimate->is_bulk,$isDiscountDisplay);
