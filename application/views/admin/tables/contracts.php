...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['tblcontracts.id', 'trash', 'client', 'hash','services']);
...
foreach ($rResult as $aRow) {
    ...
    $row[] = format_money($aRow['spare_value'], $baseCurrencySymbol);
    ...
    @$this->ci->db->from('tblstafftasks');
    @$this->ci->db->where('rel_id', $aRow['id']);
    @$this->ci->db->where('rel_type', 'contract');
    $totalTasks = @$this->ci->db->count_all_results();
    ...
    $countCompleted = @$this->ci->db->count_all_results();
    ...
    @$this->ci->db->where('rel_type', 'contract');
    @$this->ci->db->where('status !=', 5);
    $countPending = @$this->ci->db->count_all_results();
    $row[] = $countPending;
