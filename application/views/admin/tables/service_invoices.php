...
$aColumns = [
    ...
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM tbltags_in JOIN tbltags ON tbltags_in.tag_id = tbltags.id WHERE rel_id = tblserviceinvoices.id and rel_type="invoice" ORDER by tag_order ASC) as tags',
    'duedate',
    'tblserviceinvoices.status',
    'tblserviceinvoices.devide_gst as devide_gst'
];

$sIndexColumn = 'id';
$sTable = 'tblserviceinvoices';
...
$join = [
    ...
    'LEFT JOIN tblprojects ON tblprojects.id = tblserviceinvoices.project_id',
];

$custom_fields = get_table_custom_fields('service_invoice');
...
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN tblcustomfieldsvalues as ctable_' . $key . ' ON tblserviceinvoices.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
}
...
if ($this->ci->input->post('not_sent')) {
    array_push($filter, 'AND sent = 0 AND tblserviceinvoices.status NOT IN(2,5)');
}
if ($this->ci->input->post('not_have_payment')) {
    array_push($filter, 'AND tblserviceinvoices.id NOT IN(SELECT invoiceid FROM tblinvoicepaymentrecords) AND tblserviceinvoices.status != 5');
}
if ($this->ci->input->post('recurring')) {
    array_push($filter, 'AND recurring > 0');
}

$statuses = $this->ci->service_invoices_model->get_statuses();
$statusIds = [];
...
if (count($statusIds) > 0) {
    array_push($filter, 'AND tblserviceinvoices.status IN (' . implode(', ', $statusIds) . ')');
}

$agents = $this->ci->service_invoices_model->get_sale_agents();
$agentsIds = [];
...
if (count($modesIds) > 0) {
    array_push($where, 'AND tblserviceinvoices.id IN (SELECT invoiceid FROM tblinvoicepaymentrecords WHERE paymentmode IN ("' . implode('", "', $modesIds) . '"))');
}

$years = $this->ci->service_invoices_model->get_invoices_years();
$yearArray = [];
...
if ($clientid != '') {
    array_push($where, 'AND tblserviceinvoices.clientid=' . $clientid);
}
...
if (!has_permission('service_invoices', '', 'view')) {
    $userWhere = 'AND ' . get_service_invoices_where_sql_for_staff(get_staff_user_id());
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    'tblserviceinvoices.id',
    'tblserviceinvoices.clientid',
    'symbol',
    'project_id',
    'hash',
    'recurring',
    ...
    'tblserviceinvoices.sale_agent as sale_agent',
]);
...
foreach ($rResult as $aRow) {
    ...
    if (is_numeric($clientid) || $project_id) {
        $numberOutput = '<a href="' . admin_url('service_invoices/list_invoices/' . $aRow['id']) . '" target="_blank">' . format_service_invoice_number($aRow['id']) . '</a>';
    } else {
        $numberOutput = '<a href="' . admin_url('service_invoices/list_invoices/' . $aRow['id']) . '" onclick="init_service_invoice(' . $aRow['id'] . '); return false;">' . format_service_invoice_number($aRow['id']) . '</a>';
    }
...
//    $numberOutput .= '<a href="' . site_url('invoice/' . $aRow['id'] . '/' . $aRow['hash']) . '" target="_blank">' . _l('view') . '</a>';
    if (has_permission('service_invoices', '', 'edit')) {
        $numberOutput .= '<a href="' . admin_url('service_invoices/invoice/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }
    $numberOutput .= '</div>';
    ...
    $row[] = format_invoice_status($aRow['tblserviceinvoices.status']);
   /* $deliveryStatusHtml = '';
