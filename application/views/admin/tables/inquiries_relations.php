...
$aColumns = [
    ...
    'date',
    'open_till',
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM tbltags_in JOIN tbltags ON tbltags_in.tag_id = tbltags.id WHERE rel_id = tblinquiries.id and rel_type="proposal" ORDER by tag_order ASC) as tags',
    'datecreated',
    'status',
    ];
...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '<a href="' . admin_url('inquiries/list_inquiries/' . $aRow['id']) . '">'.format_inquiry_number($aRow['id']) .'</a>';

    $numberOutput .= '<div class="row-options">';

//    $numberOutput .= '<a href="' . site_url('inquiries/' . $aRow['id'] . '/' . $aRow['hash']) . '" target="_blank">' . _l('view') . '</a>';
    if (has_permission('proposals', '', 'edit')) {
        $numberOutput .= ' <a href="' . admin_url('inquiries/inquiry/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }
    $numberOutput .= '</div>';

    $row[] = $numberOutput;

    $row[] = '<a href="' . admin_url('inquiries/list_inquiries/' . $aRow['id']) . '">' . $aRow['subject'] . '</a>';
