...
$aColumns = [
    ...
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM tbltags_in JOIN tbltags ON tbltags_in.tag_id = tbltags.id WHERE rel_id = tblservicepi.id and rel_type="invoice" ORDER by tag_order ASC) as tags',
    'duedate',
    'tblservicepi.status',
    'tblservicepi.devide_gst as devide_gst'
];
...
$join = [
    'LEFT JOIN tblclients ON tblclients.userid = tblservicepi.clientid',
    'LEFT JOIN tblcurrencies ON tblcurrencies.id = tblservicepi.currency',
    'LEFT JOIN tblprojects ON tblprojects.id = tblservicepi.project_id',
];
...
if ($this->ci->input->post('not_sent')) {
    array_push($filter, 'AND sent = 0 AND tblservicepi.status NOT IN(2,5)');
}
if ($this->ci->input->post('not_have_payment')) {
    array_push($filter, 'AND tblservicepi.id NOT IN(SELECT invoiceid FROM tblinvoicepaymentrecords) AND tblservicepi.status != 5');
}
...
if (count($statusIds) > 0) {
    array_push($filter, 'AND tblservicepi.status IN (' . implode(', ', $statusIds) . ')');
}
...
if (count($modesIds) > 0) {
    array_push($where, 'AND tblservicepi.id IN (SELECT invoiceid FROM tblinvoicepaymentrecords WHERE paymentmode IN ("' . implode('", "', $modesIds) . '"))');
}
...
if ($clientid != '') {
    array_push($where, 'AND tblservicepi.clientid=' . $clientid);
}
...
if (!has_permission('service_pi', '', 'view')) {
    $userWhere = 'AND ' . get_servicepi_where_sql_for_staff(get_staff_user_id());
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    ...
    'tblservicepi.sale_agent as sale_agent',
]);
...
foreach ($rResult as $aRow) {
    ...
    if (is_numeric($clientid) || $project_id) {
        $numberOutput = '<a href="' . admin_url('service_pi/list_invoices/' . $aRow['id']) . '" target="_blank">' . format_service_pi_number($aRow['id']) . '</a>';
    } else {
        $numberOutput = '<a href="' . admin_url('service_pi/list_invoices/' . $aRow['id']) . '" onclick="init_service_pi(' . $aRow['id'] . '); return false;">' . format_service_pi_number($aRow['id']) . '</a>';
    }
...
//    $numberOutput .= '<a href="' . site_url('invoice/' . $aRow['id'] . '/' . $aRow['hash']) . '" target="_blank">' . _l('view') . '</a>';
    if (has_permission('service_pi', '', 'edit')) {
        $numberOutput .= '<a href="' . admin_url('service_pi/invoice/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }
    $numberOutput .= '</div>';
...
//    $row[] = format_money(get_invoice_total_left_to_pay($aRow['id'], $aRow['total']), $aRow['symbol']);
    ...
    $row[] = format_invoice_status($aRow['tblservicepi.status']);
    $deliveryStatusHtml = '';
