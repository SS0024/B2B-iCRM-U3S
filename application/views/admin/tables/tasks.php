...
$aColumns = [
    '1', // bulk actions
    'tblstafftasks.id as id', // bulk actions
    'name',
    '(CASE
        WHEN rel_type="contract" THEN (SELECT CASE company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM tblcontacts WHERE userid = tblclients.userid and is_primary = 1) ELSE company END company FROM tblcontracts LEFT JOIN tblclients ON tblclients.userid = tblcontracts.client WHERE tblcontracts.id = tblstafftasks.rel_id LIMIT 1)
        WHEN rel_type="customer" THEN (SELECT CASE company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM tblcontacts WHERE userid = tblclients.userid and is_primary = 1) ELSE company END company FROM tblclients where tblclients.userid = tblstafftasks.rel_id LIMIT 1)
        ELSE NULL
    ...
    '(SELECT GROUP_CONCAT(CONCAT(firstname, \' \', lastname) SEPARATOR ",") FROM tblstafftasksfollowers JOIN tblstaff ON tblstaff.staffid = tblstafftasksfollowers.staffid  WHERE taskid=tblstafftasks.id ORDER BY tblstafftasksfollowers.staffid) as followers',
...
if ($this->ci->input->post('task_type') && $this->ci->input->post('task_type') == 'contract') {
    array_push($where,'AND tblstafftasks.rel_type="contract"');
}
...
$result = data_tables_init(
    ...
    [
        ...
        '(SELECT GROUP_CONCAT(staffid SEPARATOR ",") FROM tblstafftasksfollowers WHERE taskid=tblstafftasks.id ORDER BY tblstafftasksfollowers.staffid) as followers_ids',
...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<a href="' . admin_url('clients/client/' . $aRow['client']) . '">' . $aRow['customer_name'] . '</a>';
    $row[] = ucfirst($bilableArray[$aRow['billable']]);
    $row[] = ucfirst($aRow['rel_type']);

    $canChangeStatus = ($aRow['current_user_is_creator'] != '0' || has_permission('tasks', '', 'edit'));
...
*/
    ...
    $row[] = format_members_by_ids_and_names($aRow['followers_ids'], $aRow['followers']);
