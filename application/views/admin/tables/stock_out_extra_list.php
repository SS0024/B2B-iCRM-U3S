...
$where = [];
array_push($where, 'AND tbldelivery_challans.status NOT IN (5)');
...
foreach ($rResult as $aRow) {
    ...
    if(has_permission('delivery_challan', '', 'view')){
        $numberOutput = '<a href="' . admin_url('delivery_challan/list_invoices/' . $aRow['id']) . '"  >' . format_delivery_challan_number($aRow['id']) . '</a>';
    }else{
        $numberOutput = '<a href="javascript:void(0)"  >' . format_delivery_challan_number($aRow['id']) . '</a>';
    }

    $row[] = $numberOutput;
    $row[] = _dt($aRow['date']);
    if ($aRow['is_stock_out'] == 2) {
        $row[] = icon_btn(admin_url('delivery_challan/stock_out_pdf/' . $aRow['id']), 'file-pdf-o');
    }elseif(has_permission('stock_out', '', 'create')){
        $row[] = '<a href="' . admin_url('delivery_challan/stock_out/' . $aRow['id']) . '" class="btn btn-default btn-icon" >Stock Out</a>';
    }
