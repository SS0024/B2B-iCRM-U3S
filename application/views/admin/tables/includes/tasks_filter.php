...
if ($this->ci->input->post('customers')) {
    $customers = implode(",", $this->ci->input->post('customers'));

    array_push($filter, 'AND  (CASE 
        WHEN rel_type="contract" and (SELECT client FROM tblcontracts WHERE tblcontracts.id = tblstafftasks.rel_id limit 1) in ('.$customers.') THEN true
        WHEN rel_type="customer" and tblstafftasks.rel_id in ('.$customers.') THEN true
        ELSE false
        END)');
}
