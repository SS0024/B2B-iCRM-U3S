...
$agents    = $this->ci->expenses_model->get_sale_agents();
$agentsIds = [];
foreach ($agents as $agent) {
    if ($this->ci->input->post('sale_agent_' . $agent['addedfrom'])) {
        array_push($agentsIds, $agent['addedfrom']);
    }
}
if (count($agentsIds) > 0) {
    array_push($filter, 'AND tblexpenses.addedfrom IN (' . implode(', ', $agentsIds) . ')');
}
