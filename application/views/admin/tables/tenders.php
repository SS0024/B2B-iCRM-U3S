...
$aColumns = [
    ...
    'tbltenders.adminnote as adminnote',
    'open_till',
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM tbltags_in JOIN tbltags ON tbltags_in.tag_id = tbltags.id WHERE rel_id = tbltenders.id and rel_type="proposal" ORDER by tag_order ASC) as tags',
    'datecreated',
    'status',
    'lead_status',
    'tbltenders.assigned as assigned',
	'tbltenders.devide_gst as devide_gst',
	'tbltenders.id'
];
...
$statuses  = $this->ci->tenders_model->get_statuses();
$statusIds = [];
...
$agents    = $this->ci->tenders_model->get_sale_agents();
$agentsIds = [];
...
$years      = $this->ci->tenders_model->get_proposals_years();
$yearsArray = [];
...
if (!has_permission('tenders', '', 'view')) {
    array_push($where, 'AND ' . get_tenders_sql_where_staff(get_staff_user_id()));
}
if(isset($_POST['contact_person']) && !empty($_POST['contact_person'])){
    ...
    if(!empty($estimateIds)){
        array_push($where, 'AND tbltenders.id IN ('.$estimateIds.')');
    }else{
        array_push($where, 'AND tbltenders.id IN (0)');
    }
}
...
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN tblcustomfieldsvalues as ctable_' . $key . ' ON tbltenders.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
}
...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '<a href="' . admin_url('tenders/list_tenders/' . $aRow['tbltenders.id']) . '" onclick="init_tender(' . $aRow['tbltenders.id'] . '); return false;">'.$aRow['tender_number'] .'</a>';

    $numberOutput .= '<div class="row-options">';
    if (has_permission('tenders', '', 'edit')) {
        $numberOutput .= '<a href="' . admin_url('tenders/tender/' . $aRow['tbltenders.id']) . '">' . _l('edit') . '</a>';
    }
    $numberOutput .= '</div>';
    ...
    $contactPerson = [];
    $divisions = [];
    $divConArray = get_div_cons_by_type('inquiry', $aRow['tbltenders.id']);
    ...
    $estimateItem = get_items_by_type('inquiry', $aRow['tbltenders.id'])[0];
    @$this->ci->db->select('*');
