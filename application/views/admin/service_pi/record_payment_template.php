<div class="col-md-12 no-padding animated fadeIn">
    <div class="panel_s">
        <?php echo form_open_multipart('admin/service_invoices/record_payment',array('id'=>'record_payment_form','class'=>'dropzone dropzone-manual')); ?>
        <?php echo form_hidden('invoiceid',$invoice->id); ?>		<?php echo form_hidden('temp_file_id',0,array('id' =>'temp_file_id')); ?>
        <div class="panel-body">
            <h4 class="no-margin"><?php echo _l('record_payment_for_invoice'); ?> <?php echo format_service_invoice_number($invoice->id); ?></h4>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <select class="selectpicker" name="paymentmode" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                            ...
                            <?php if(is_payment_mode_allowed_for_service_invoice($mode['id'],$invoice->id)){ ?>
   ...
   $(function(){	 		 	 	 
	 ...
	 if($('#dropzoneDragArea').length > 0){        
		...
		paymentDropzone = new Dropzone("#record_payment_form",  $.extend({},_dropzone_defaults(),{         
			...
			url:admin_url + 'service_invoices/add_invoice_payment_attachment/',
			success:function(file,response){           
				...
				if (response.id) {			  
					...
					window.location.assign(admin_url + 'service_invoices/list_invoices#'+invoice_id);
...
function delete_temp_payment_attachment(wrapper, id) {
    if (confirm_delete()) {
        requestGetJSON(admin_url+'service_invoices/delete_temp_payment_attachment/' + id).done(function(response) {
