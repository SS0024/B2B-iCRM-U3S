						 ...
						 <div class="row">
							...
							?>
							 ...
							 <div class="col-md-2 text-right">
								<a href="<?php echo admin_url('invoices/delete_delivery_module_attachment/'.$delivery_detail->id.'/'.$delivery_detail->invoice_id); ?>" class="text-danger _delete"><i class="fa fa fa-times"></i></a>
							 </div>
							 <?php } ?>
						 </div>
					</div>
        ...
        if (response.delivery_moduleid) {
         if(typeof(expenseDropzone) !== 'undefined'){
          if (expenseDropzone.getQueuedFiles().length > 0) {
            expenseDropzone.options.url = admin_url + 'invoices/add_delivery_module_attachment/' + response.delivery_moduleid+'/'+response.invoice_id;
