    ...
    <div class="table-responsive s_table">
        <table class="table estimate-items-table items table-main-estimate-edit no-mtop">
            ...
            <tr>
                ...
                ?>
                <!--  <th width="25%" align="left"><?php echo 'Code'; ?>
              <th width="25%" align="left"><?php echo 'HSN'; ?> -->
                <th width="10%" class="qty" align="right"><?php echo $qty_heading; ?></th>
                <?php
                if(isset($estimate)){
                    ?>
                    <th width="5%" class="qty" align="right">Hold <?php echo $qty_heading; ?></th>
                    <?php
                }
                ?>
                <th width="10%" align="right"><?php echo _l('estimate_table_item_amount_heading'); ?></th>
                <th width="10%" align="right"><?php echo _l('estimate_table_item_discount_heading'); ?></th>
            ...
            <tr class="main">
                <td></td>
                <td>
                    <input type="hidden" class="main itemid" name="itemid" id="itemid">
                ...
                <!-- <td>
                  <input type="text" name="hsn" rows="4" class="form-control" placeholder="<?php echo 'hsn'; ?>">
               </td>
                <td>
                  <input type="text" name="code" rows="4" class="form-control" placeholder="<?php echo 'code'; ?>">
               </td> -->
                ...
                <td>
                    ...
                    $select .= '<option value="">Select Tax</option>';
                ...
                <td>
                    ...
                    ?>
                    <button type="button"
                            onclick="add_item_to_table('undefined','undefined',<?php echo $new_item; ?>,'',document.getElementById('itemid').value); return false;"
            ...
            <?php if (isset($estimate) || isset($add_items)) {
                ...
                foreach ($add_items as $item) {
                    ...
                    $estimate_item_taxes = get_purchaseorder_item_taxes($item['id']);
                    if ($item['id'] == 0) {
                    ...
                    $maxVal = ($item['qty'] - $item['converted_qty']) > $stock ? $stock : ($item['qty'] - $item['converted_qty']);

                    $table_row .= '<td><input type="number" min="0" onblur="calculate_total();" onchange="calculate_total();" readonly data-quantity name="' . $items_indicator . '[' . $i . '][qty]" value="' . $item['qty'] . '" class="form-control">';
                    $unit_placeholder = '';
                    if (!$item['unit']) {
                    ...
                    $table_row .= '<label id="stockdetails">Stock : '.$stock.'</label></td>';
                    if (isset($estimate)) {
                        $table_row .= '<td><input type="number" min="0" max="'.$maxVal.'" onblur="calculate_total();" onchange="calculate_total();" data-quantity="' . round($item['hold_item']) . '" name="' . $items_indicator . '[' . $i . '][quantity_received]" value="' . round($item['hold_item']) . '" class="form-control received_qty"><label >Converted Qty : '.$item['converted_qty'].'</label>';
                        $table_row .= '</td>';
                    }

                    $table_row .= '<td class="item_amount"><input type="number" data-toggle="tooltip" title="' . _l('numbers_not_formatted_while_editing') . '" onblur="calculate_total();" onchange="calculate_total();" name="' . $items_indicator . '[' . $i . '][item_amount]" value="' . $item['item_amount'] . '" class="form-control" min="0"></td>';

                    $table_row .= '<td class="item_discount"><div class="input-group"><input type="number" data-toggle="tooltip" title="' . _l('numbers_not_formatted_while_editing') . '" onblur="calculate_total();" onchange="calculate_total();" name="' . $items_indicator . '[' . $i . '][item_discount]" value="' . $item['item_discount'] . '" class="form-control" min="0"><div class="input-group-addon"><span class="discount-total-type-selected">%</span></div></div></td>';

                    $table_row .= '<td class="rate"><input type="number" data-toggle="tooltip" title="' . _l('numbers_not_formatted_while_editing') . '" onblur="calculate_total();" onchange="calculate_total();" name="' . $items_indicator . '[' . $i . '][rate]" value="' . $item['rate'] . '" class="form-control" readonly="readonly"></td>';
                    $table_row .= '<td class="taxrate">' . $this->misc_model->get_taxes_dropdown_template('' . $items_indicator . '[' . $i . '][taxname][]', $estimate_item_taxes, 'purchaseorder', $item['id'], true, $manual) . '</td>';
