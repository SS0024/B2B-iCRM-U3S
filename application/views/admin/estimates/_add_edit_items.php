        ...
        <table class="table estimate-items-table items table-main-estimate-edit no-mtop">
            ...
            <tr class="main">
                ...
                <td>
                    ...
                    <?php
                    if (isset($estimate) || $bodyclass == 'estimate') {
                        ?>
                        <select class="selectpicker display-block mtop2" name="item_other_status" data-width="100%"
                                data-none-selected-text="Select product status">
                            <option value="">Select product status</option>
                            <option value="lost">Declined</option>
                            <option value="won">Accepted</option>
                        </select>
                        <input name="item_remark" value="" class="form-control mtop2" placeholder="Remarks">
                        <?php
                    }
                    ?>
                </td>
            ...
            <?php if (isset($estimate) || isset($add_items) || isset($invoice)) {
                ...
                foreach ($add_items as $item) {
                    ...
                    if (isset($estimate) && $bodyclass == 'inquiry') {
                        $estimate_item_taxes = get_inquiry_item_taxes($item['id']);
                    } elseif ($item['id'] == 0) {
                        $estimate_item_taxes = $item['taxname'];
                        $manual = true;
                    } elseif (!isset($is_proposal)) {
                    ...
                    if (isset($invoice) && $bodyclass == 'delivery_challan') {
                        $estimate_item_taxes = get_delivery_challan_item_taxes($item['id']);;
                    }
                    if ($item['id'] == 0) {
                        $estimate_item_taxes = $item['taxname'];
                        $manual = true;
                    ...
                    if (isset($estimate) || $bodyclass == 'estimate') {
                        if ($item['item_other_status'] == 'lostConvert' || $item['item_other_status'] == 'wonConvert'){
                            $table_row .='<label>'.($item['item_other_status'] == 'lostConvert' ? 'Declined' : 'Accepted').'</label>
                                        <input type="hidden" name="' . $items_indicator . '[' . $i . '][item_other_status]" value="'.$item['item_other_status'].'">
                            ...
                            $table_row .= '<select class="selectpicker display-block mtop2" '.($item['item_other_status'] == 'lostConvert' || $item['item_other_status'] == 'wonConvert' ? "readonly" : "").'  name="' . $items_indicator . '[' . $i . '][item_other_status]" onblur="calculate_total();" onchange="calculate_total();"  data-width="100%" data-none-selected-text="Select product status">
                               <option value="">Select product status</option>
                               <option ' . ($item['item_other_status'] == 'lost' ? 'selected="selected"' : '') . '  value="lost">Declined</option>
                               <option ' . ($item['item_other_status'] == 'won' ? 'selected="selected"' : '') . ' value="won">Accepted</option>
                           </select>
                           <input name="' . $items_indicator . '[' . $i . '][item_remark]" value="' . $item["item_remark"] . '" onblur="calculate_total();" onchange="calculate_total();"  class="form-control mtop2" placeholder="Remarks">';
                        }
                    }
                    ...
                    $table_row .= '<td><input type="number" min="0" onblur="calculate_total();" onchange="calculate_total();" data-quantity="' . $item['qty'] . '" name="' . $items_indicator . '[' . $i . '][qty]" value="' . $item['qty'] . '" class="form-control">';
                    $unit_placeholder = '';
                    if (!$item['unit']) {
                    ...
                    if (isset($estimate) && $bodyclass == 'inquiry') {
                        $table_row .= '<td class="item_amount pro_amt_td"><div class="input-group"><input type="number" data-toggle="tooltip" title="' . _l('numbers_not_formatted_while_editing') . '" onblur="calculate_total();" onchange="calculate_total();" name="' . $items_indicator . '[' . $i . '][item_amount]" value="' . $item['item_amount'] . '" class="form-control" min="0"><div class="input-group-addon"><a class="discount-total-type-selected" data-toggle="tooltip" title="' . _l('Refresh Price') . '" href="javascript:void(0);" onclick="getLatestPrice(\''.$item['description'].'\',\''.$i.'\')"><i class="fa fa-refresh"></i></a></div></div></td>';
                    }else{
                    ...
                    $table_row .= '<td class="taxrate">' . $this->misc_model->get_taxes_dropdown_template('' . $items_indicator . '[' . $i . '][taxname][]', $estimate_item_taxes, (isset($is_proposal) ? 'proposal' : ($bodyclass == 'inquiry' ? 'inquiry': 'estimate')), $item['id'], true, $manual) . '</td>';
                        ...
                        <div class="col-md-5">
                            ...
                            <input type="number" data-toggle="tooltip"
                                   ...
                                   value="<?php if (isset($estimate)) {
                                       ...
                                       echo 0;
                                   } ?>" class="form-control pull-left" required name="servicecharge">
                        </div>
                    </div>
                </td>
