...
<?php echo form_hidden('_attachment_sale_type', 'requsite_form'); ?>
<div class="col-md-12 no-padding">
    <div class="panel_s">
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-md-3">
                    <span class="label label-default default s-status mtop5 estimate-status-<?= $estimate->status; ?> estimate-status-default"><?= $estimate_statuses[$estimate->status];  ?></span>
<!--                    --><?php //echo format_estimate_status($estimate->status,'mtop5');  ?>
                ...
                <div class="col-md-9">
                    ...
                    <div class="pull-right _buttons">
                        <?php if (has_permission('requisite_forms', '', 'edit')) { ?>
                            <a href="<?php echo admin_url('requsite_forms/requsite_form/' . $estimate->id); ?>"
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li class="hidden-xs"><a
                                            href="<?php echo admin_url('requsite_forms/pdf/' . $estimate->id . '?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a>
                                </li>
                                <li class="hidden-xs"><a
                                            href="<?php echo admin_url('requsite_forms/pdf/' . $estimate->id . '?output_type=I'); ?>"
                                            target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
                                <li>
                                    <a href="<?php echo admin_url('requsite_forms/pdf/' . $estimate->id); ?>"><?php echo _l('download'); ?></a>
                                </li>
                                <li>
                                <li>
                                    <a href="<?php echo admin_url('requsite_forms/pdf/' . $estimate->id . '?print=true'); ?>"
                                       target="_blank">
                                        <?php echo _l('print'); ?>
                                    </a>
                                </li>
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                ...
                                <?php if (has_permission('requisite_forms', '', 'delete')) { ?>
                                    <li>
                                        <a href="<?php echo admin_url('requsite_forms/delete/' . $estimate->id); ?>"
                                           class="text-danger delete-text _delete"><?php echo _l('Delete Requsite form'); ?></a>
                                    </li>
                                    <?php
                                }
                                ?>
                        ...
                        <?php if ($estimate->invoiceid == NULL) { ?>
                            <?php if (has_permission('estimates', '', 'create') && !empty($estimate->clientid)) { ?>
                                <div class="btn-group pull-right mleft5">
                                    <button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown"
                                            aria-haspopup="true" aria-expanded="false">
                                        <?php /*echo _l('estimate_convert_to_invoice');*/
                                        echo 'Convert' ?> <span class="caret"></span>
                                    </button>
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a href="<?php echo admin_url('requsite_forms/convert_to_quotation/' . $estimate->id . '?save_as_draft=true'); ?>"><?php echo _l('Convert to Quotation'); ?></a>
                                        </li>
                                        <!--<li>
                                            <a href="<?php /*echo admin_url('requsite_forms/convert_to_invoice/' . $estimate->id); */?>"><?php /*echo _l('convert') . ' Invoice'; */?></a>
                                        </li>
                                        <li class="divider"></li>
                                        <li>
                                            <a href="<?php /*echo admin_url('purchaseorder/copy_from_estimate/' . $estimate->id); */?>"><?php /*echo _l('convert') . ' PO'; */?></a>
                                        </li>-->
                                    </ul>
                                </div>
                            <?php } ?>
                        <?php } else { ?>
                            <a href="<?php echo admin_url('estimates/list_estimates/' . $estimate->invoice->id); ?>"
                               data-placement="bottom" data-toggle="tooltip"
                               title="<?php echo _l('Requsite form Quotation on '._dt($estimate->invoiced_date)); ?>"
                               class="btn mleft10 btn-info"><?php echo format_estimate_number($estimate->invoice->id); ?></a>
                        <?php } ?>
                    </div>
                </div>
            </div>
                ...
                <div role="tabpanel" class="tab-pane ptop10 active" id="tab_estimate">
                    <div id="estimate-preview">
                        <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="alert alert-info mbot15">
                                        <?php echo _l('accepted_identity_info', array(
                                            ...
                                            '<b>' . $estimate->acceptance_ip . '</b>' . (is_admin() ? '&nbsp;<a href="' . admin_url('requsite_forms/clear_acceptance_info/' . $estimate->id) . '" class="_delete text-muted" data-toggle="tooltip" data-title="' . _l('clear_this_information') . '"><i class="fa fa-remove"></i></a>' : '')
                                        )); ?>
                                    </div>
                                </div>
                            <?php } ?>
                           ...
                           <?php echo format_requsite_form_number($estimate->id); ?>
                           </span>
                                    </a>
                                </h4>
                                <address>
                           ...
                           <?php echo _l('Requsit form date'); ?>:
                                ...
                                <?php } ?>
                                <?php $pdf_custom_fields = get_custom_fields('requsite_form', array('show_on_pdf' => 1));
                                foreach ($pdf_custom_fields as $field) {
                                    $value = get_custom_field_value($estimate->id, $field['id'], 'requsite_form');
                                    if ($value == '') {
                                        continue;
                                    } ?>
                                    <p class="no-mbot">
...
//                                    $items_data = get_table_items_and_taxes($estimate->items, 'estimate', true);
                                            ...
                                            <?php
                                                ...
                                                foreach ($estimate->items as $item){
                                                    ...
                                                    <tr class="sortable" data-item-id="<?= $item['id'] ?>">
                                                        ...
                                                        <td class="description" align="left;"><span style="font-size:14px;">
                                                                <strong><?= $item['description'] ?></strong></span><br><span
                                                                    style="color:#424242;"><?= $item['long_description'] ?></span></td>
                                                        <td align="right"><?= floatVal($item['qty'])." ".$item['unit'] ?></td>
                                                    </tr>
                                                    <?php
                                                    $i++;
                                                }
                ...
                <div role="tabpanel" class="tab-pane" id="tab_attachments">
                    <?php if (count($estimate->attachments) > 0) { ?>
                        <div class="col-md-12">
                            <p class="bold text-muted"><?php echo _l('Requisite Form Files'); ?></p>
                        </div>
                ...
                <div role="tabpanel" class="tab-pane" id="tab_notes">
                    <?php echo form_open(admin_url('requsite_forms/add_note/' . $estimate->id), array('id' => 'sales-notes', 'class' => 'estimate-notes-form')); ?>
...
<?php $this->load->view('admin/requsite_forms/estimate_send_to_client'); ?>
