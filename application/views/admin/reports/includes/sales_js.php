<script>
 ...
 var report_transaction_items = $('#items-transaction-report');
 ...
 function init_report(e, type) {
   ...
   report_transaction_items.addClass('hide');
   ...
   // Clear custom date picker
      ...
      }else if(type == 'items-transaction-report'){
           report_transaction_items.removeClass('hide');
      }
      gen_reports();
    }
 ...
 function items_transaction_report(){
   if ($.fn.DataTable.isDataTable('.table-items-transaction-report')) {
     $('.table-items-transaction-report').DataTable().destroy();
   }
   initDataTable('.table-items-transaction-report', admin_url + 'reports/items_transaction', [2,3,4,5], [2,3,4,5], fnServerParams, [0, 'asc']);
 }
   ...
   function gen_reports() {
     ...
     if (!$('.chart-income').hasClass('hide')) {
      ...
      items_report();
    } else if(!report_transaction_items.hasClass('hide')) {
      items_transaction_report();
