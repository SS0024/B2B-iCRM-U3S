<div class="col-md-12 no-padding animated fadeIn">
    <div class="panel_s">
        <?php echo form_open_multipart('admin/delivery_challan/record_payment',array('id'=>'record_payment_form','class'=>'dropzone dropzone-manual')); ?>
   ...
   $(function(){	 		 	 	 
	 ...
	 if($('#dropzoneDragArea').length > 0){        
		...
		paymentDropzone = new Dropzone("#record_payment_form",  $.extend({},_dropzone_defaults(),{         
			...
			url:admin_url + 'delivery_challan/add_invoice_payment_attachment/',
			success:function(file,response){           
				...
				if (response.id) {			  
					...
					window.location.assign(admin_url + 'delivery_challan/list_invoices#'+invoice_id);
...
function delete_temp_payment_attachment(wrapper, id) {
    if (confirm_delete()) {
        requestGetJSON(admin_url+'delivery_challan/delete_temp_payment_attachment/' + id).done(function(response) {
