    ...
    <div class="row">
            ...
            <b>
               <?php if ($recurring_invoice->cycles == 0) {
                ...
                <?php if ($invoice->is_recurring_from != NULL) { ?>
                    <?php echo '<p class="text-muted mtop15">' . _l('invoice_recurring_from', '<a href="' . admin_url('invoices/list_invoices/' . $invoice->is_recurring_from) . '" onclick="init_invoice(' . $invoice->is_recurring_from . ');return false;">' . format_purchase_number($invoice->is_recurring_from) . '</a></p>'); ?>
                <?php } ?>
            </div>
        ...
        <div class="col-md-6 col-sm-6">
            <h4 class="bold">
                ...
                ?>
                <a href="<?php echo admin_url('purchases/purchase/' . $invoice->id); ?>">
            ...
            <address>
                <b style="color:black" class="company-name-formatted"><?= $invoice->supplier ?></b><br> <?= $invoice->supplier_address ?><br>
                <?= $invoice->supplier_city ?>-<?= $invoice->supplier_zip ?>, <?= $invoice->supplier_state ?><br> India <br> GST Number: <?= $invoice->supplier_gst_number ?>
            </address>
        ...
        <div class="col-sm-6 text-right">
         ...
         <address>
            ...
            if (isset($invoices_delivery_status) && !empty($invoices_delivery_status) && isset($invoices_delivery_status->name)) { ?>
                <p class="no-mbot">
            <span class="bold">
            ...
            <div class="table-responsive">
                ...
                <table class="table items invoice-items-preview">
                    ...
                    <tr>
                        ...
                        <th align="right"><?php echo $qty_heading; ?> Received</th>
                        <th align="right">Balance <?php echo $qty_heading; ?></th>
                    ...
                    foreach ($invoice->items as $item) {
                        ...
                        $item_taxes = $this->db->get('tblitemstax')->result_array();

                        $k = 0;
                        foreach ($taxes as $tax) {
                            $item_taxes[$k]['taxname'] = $tax['taxname'] . '|' . $tax['taxrate'];
                            $item_taxes[$k]['onlytaxname'] = $tax['taxname'];
                            $item_taxes[$k]['taxnum'] = $tax['taxrate'] + 0;
                            $k++;
                        }
                        ...
                        if (count($item_taxes) > 0) {
                            ...
                            foreach ($item_taxes as $tax) {
                                $calc_tax = 0;
                                $tax_not_calc = false;
                                $ammnt = $item['quantity'] * $item['real_unit_cost'];
                                $ttl_tx = ($tax['taxrate'] / 100) * $ammnt;

                                /* $tax_qty[$tax['taxnum']['total_qtys']] +=  $item['quantity'];
                                ...
                                if ($tax_not_calc == true) {
                                    ...
                                    array_push($result['taxes'][$tax['taxname']]['total'], (($item['quantity'] * $item['real_unit_cost']) / 100 * $tax['taxrate']));
                        ...
                        <tr class="sortable" style="<?= $statusColor ?>" data-item-id="<?= $item['product_id'] ?>">
                            ...
                            <td align="right"><?= round($item['quantity_received']) ?></td>
                            <td align="right"><?= round($item['quantity_balance']) ?></td>
                            <td align="right"><?= _format_number($item['real_unit_cost']) ?></td>
                            <?= $itemHTML ?>
