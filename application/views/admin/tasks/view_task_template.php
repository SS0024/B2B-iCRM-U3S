...
<div class="modal-body">
    <div class="row">
        <div class="col-md-8 task-single-col-left">
            ...
            <?php if ($task->status != 5 && (is_admin() || ($task->current_user_is_assigned  && !empty(format_service_pi_number($task->service_invoice_id))) || $task->current_user_is_creator)) { ?>
                <?php
                if ($task->billable != 1 || ($task->billable == 1 && $task->service_invoice_id != 0 && isset($task->requsite_form_id))) {
                    ?>
            ...
            <?php if ($task->billed == 0) {
                ...
                if (!$this->tasks_model->is_timer_started($task->id)) { ?>
                    <p class="no-margin pull-left"<?php if (!$is_assigned) { ?> data-toggle="tooltip" data-title="<?php echo _l('task_start_timer_only_assignee'); ?>"<?php } ?>>
                        <a href="#" class="mbot10 btn<?php if (!$is_assigned || $task->status == 5) {
                ...
                <?php }
                    if ($task->billable == 1 && ($task->rel_type == 'contract' || $task->rel_type == 'customer')) {
                        ?>
                        <p class="no-margin pull-left">
                            <?php
                            if ($task->service_invoice_id != 0) {
                                if (!empty(format_service_pi_number($task->service_invoice_id))) {
                                    ?>
                                    <a
                                            href="<?php echo admin_url('service_pi/list_invoices/' . $task->service_invoice_id); ?>"
                                            data-placement="<?php echo is_mobile() ? 'bottom' : 'right'; ?>"
                                            data-html="true"
                                            data-trigger="manual" class="btn mbot10 btn-info">
                                        <?php echo format_service_pi_number($task->service_invoice_id); ?>
                                    </a>
                                    <?php
                                }
                            } else {
                                ...
                                &nbsp;&nbsp;<a
                                        <?php
                                        if(is_admin() || $task->current_user_is_creator){
                                            ?>
                                            href="<?php echo admin_url('service_pi/invoice'); ?>?task_id=<?= $task->id ?>"
                                            <?php
                                        }else{
                                            ?>
                                            href="#"
                                        ...
                                        data-placement="<?php echo is_mobile() ? 'bottom' : 'right'; ?>"
                                        data-html="true"
                                        data-trigger="manual" class="btn mbot10 btn-danger">
                                    <?php echo _l('Generate Service Pi'); ?>
                                </a>
                                <?php
                            }
                            ?>
            ...
            <div class="clearfix"></div>
            <?php
            if ($task->rel_type == 'contract' || $task->rel_type == 'customer') {
                if ($task->requsite_form_id != 0) {
                    if (!empty(format_requsite_form_number($task->requsite_form_id))) {
                        ?>
                        <a href="<?php echo admin_url('requsite_forms/list_requsite_forms/' . $task->requsite_form_id); ?>"
                           data-placement="<?php echo is_mobile() ? 'bottom' : 'right'; ?>" data-html="true"
                           data-trigger="manual" class="btn mbot10 btn-default">
                            <?php echo format_requsite_form_number($task->requsite_form_id); ?>
                        </a>
                        <?php
                    }
                } else {
                    ?>
                    <div class="mtop10">
                        <span><b><?php echo _l('Is have Requsite form'); ?> </b></span><br/>
                        <div class="radio radio-primary radio-inline">
                            <input type="radio" value="1" id="sq_1" name="is_requsite"
                                   onclick="changeRequsiteOptionValue(<?= $task->id ?>,1);"
                                   data-text="<?php echo _l('Yes'); ?>" <?php if (isset($task->is_requsite) && $task->is_requsite == 1) {
                                echo 'checked';
                            } ?>>
                            <label for="sq_1"><?php echo _l('Yes'); ?></label>
                        </div>
                        <div class="radio radio-primary radio-inline">
                            <input type="radio" value="0" id="sq_2" name="is_requsite"
                                   onclick="changeRequsiteOptionValue(<?= $task->id ?>,0);"
                                   data-text="<?php echo _l('No'); ?>" <?php if (isset($task->is_requsite) && $task->is_requsite == 0) {
                                echo 'checked';
                            } ?>>
                            <label for="sq_2"><?php echo _l('No'); ?></label>
                        </div>
                    </div>
                    <?php
                }
                        ...
                        <div class="row">
                            ...
                            foreach ($task->attachments as $attachment) { ?>
                                ...
                                <div data-num="<?php echo $i; ?>"
                                    ...
                                    <ul class="list-unstyled task-attachment-wrapper" data-toggle="tooltip"
       ...
       <button type="button" class="btn btn-default pull-right mright5" onclick="cancel_edit_comment(' . $comment['id'] . ')">' . _l('cancel') . '</button>
        ...
        <div class="col-md-4 task-single-col-right">
            ...
            <div class="task-info task-status task-info-status">
                <h5>
                    <i class="fa fa-tasks pull-left task-info-icon fa-fw fa-lg"></i>
                    Task ID: <?= $task->id ?>
                </h5>
            </div>
            <div class="task-info task-status task-info-status">
                <h5>
                    <i class="fa fa-<?php if ($task->status == 5) {
                        echo 'star';
            ...
            <?php if ((has_permission('tasks', '', 'create') || has_permission('tasks', '', 'edit'))) { ?>
                ...
                <div class="task-info task-info-billable">
                    <h5><i class="fa task-info-icon fa-fw fa-lg pull-left fa fa-credit-card"></i>
                        ...
                        echo $bilableArray[$task->billable];
                        ?>
...
<script>
    ...
    function changeRequsiteOptionValue(id, value) {
        if (value == 1) {
            window.open(buildUrl(admin_url + 'requsite_forms/requsite_form/?task_id=' + id), '_self');
        } else {
            requestGetJSON('tasks/changeRequsiteOptionValue/' + id + '/' + value).done(function (response) {
            });
        }
    }
