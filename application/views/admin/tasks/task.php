         ...
         <div class="row">
            <div class="col-md-12">
               ...
               <div class="checkbox checkbox-primary no-mtop checkbox-inline task-add-edit-public">
                ...
                $bilableArray = [
                    ['id'=>10,'name'=>'None'],
                    ['id'=>1,'name'=>'Out of Warranty'],
                    ['id'=>2,'name'=>'Warranty'],
                    ['id'=>3,'name'=>'FOC'],
                    ['id'=>4,'name'=>'AMC']
                ];
                echo render_select('billable', $bilableArray, array('id', 'name'), '', $task->billable);
                ?>
               ...
               if(isset($service_no)){
                   echo form_hidden('service_no',$service_no);
                   $value="$service_no ".(isset($hour)? "| $hour HR" : '').(isset($service_details->item) ? '| FAB No.:'.$service_details->item->fab_no : '');
               }
               if($value == '' && isset($subject) && $subject !=''){
                   $value =$subject;
               }
               ?>
                  ...
                  <div class="col-md-6">
                     <?php if(isset($task)){
                        ...
                        if (isset($service_details) && !empty($service_details)){
                            $date_attrs['data-date-min-date'] = $service_details->datestart;
                            $date_attrs['data-date-end-date'] = $service_details->dateend;
                            $project_end_date_attrs['data-date-min-date'] = $service_details->datestart;
                            $project_end_date_attrs['data-date-end-date'] = $service_details->dateend;
                        }
                        ?>
                     <?php echo render_date_input('startdate','task_add_edit_start_date',$value, $date_attrs); ?>
                  </div>
                  <div class="col-md-6">
                     <?php $value = (isset($task) ? _d($task->duedate) : (isset($due_date) ? _d($due_date) : '')); ?>
                  ...
                  <div class="col-md-6">
                     <div class="form-group">
                        ...
                        <select name="rel_type" class="selectpicker" id="rel_type" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                           ...
                           <option value="inquiry" <?php if(isset($task) || $this->input->get('rel_type')){if($rel_type == 'inquiry'){echo 'selected';}} ?>>
                              <?php echo _l('inquiry'); ?>
                           </option>
                           <option value="tender" <?php if(isset($task) || $this->input->get('rel_type')){if($rel_type == 'tender'){echo 'selected';}} ?>>
                              <?php echo _l('Tender'); ?>
                           </option>
                        </select>
                     </div>
                ...
                if(isset($task) || $this->input->get('rel_type')){
                    if($rel_type == 'contract') {
                        ...
                        <div class="form-group">
                            ...
                            <select name="consumables[]" class="selectpicker" multiple data-width="100%"
                                    data-none-selected-text="<?php echo _l('Select Consumable'); ?>">
                                <?php
                                $consumables = isset($task->consumables) ? explode(',', $task->consumables) : array();
