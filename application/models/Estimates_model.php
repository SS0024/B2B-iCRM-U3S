...
class Estimates_model extends CRM_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($estimate) {
                ...
                $estimate->purchaseorders = $this->get_purchaseorders($id);
    ...
    public function add($data)
    {
        ...
        if (isset($data['expirydate'])){
            $timestamp = strtotime('+4 days', strtotime(to_sql_date($data['expirydate'])));
            $data['follow_up_date'] = date('Y-m-d H:i:s',$timestamp);
        }
		...
		if(get_option('next_estimate_number') != $data['number'] && get_option('next_estimate_number') > $data['number']){
            $data['number'] = get_option('next_estimate_number');
        }
        ...
        if ($insert_id) {
            ...
            foreach ($div_cons as $con => $div) {
                if (isset($div)) {
                    add_new_div_con_item_post($div, $con, $insert_id, 'estimate');
                }
            }
            foreach ($items as $key => $item) {
    ...
    public function update($data, $id)
    {
        ...
        $data = $this->map_shipping_columns($data);
        if (!empty($div_cons)) {
            handle_removed_div_cons_post($id, 'estimate');
            foreach ($div_cons as $con => $div) {
                if (isset($div)) {
                    add_new_div_con_item_post($div, $con, $id, 'estimate');
                }
            }
        }
