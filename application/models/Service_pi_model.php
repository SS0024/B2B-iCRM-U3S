...
class Service_pi_model extends CRM_Model
{
    ...
    {
        return $this->db->query("SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, ' ', lastname) as full_name FROM tblservicepi JOIN tblstaff ON tblstaff.staffid=tblservicepi.sale_agent WHERE sale_agent != 0")->result_array();
    }
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->log_invoice_activity($id, 'unmarked service pi as cancelled');
    ...
    public function get_invoice_recurring_invoices($id)
    {
        ...
        $invoices           = $this->db->get('tblservicepi')->result_array();
        $recurring_invoices = [];
    ...
    {
        $this->db->select('*, tblcurrencies.id as currencyid, tblservicepi.id as id, tblcurrencies.name as currency_name');
        $this->db->from('tblservicepi');
        $this->db->join('tblcurrencies', 'tblcurrencies.id = tblservicepi.currency', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where('tblservicepi' . '.id', $id);
            $invoice = $this->db->get()->row();
            if ($invoice) {
                $invoice->items       = get_items_by_type('service_pi', $id);
                ...
                $invoice->payments = $this->invoice_payments_model->get_invoice_payments($id);
            }
    ...
    public function get_attachments($invoiceid, $id = '')
    {
        ...
        $this->db->where('rel_type', 'service_pi');
    ...
    public function get_invoices_total($data)
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            $select = 'id,total';
            if ($i == 1) {
                $select .= ', (SELECT total - (SELECT COALESCE(SUM(amount),0) FROM tblinvoiceservicepaymentrecords WHERE invoiceid = tblservicepi.id) - (SELECT COALESCE(SUM(amount),0) FROM tblcredits WHERE tblcredits.invoice_id=tblservicepi.id)) as outstanding';
            } elseif ($i == 2) {
                $select .= ',(SELECT SUM(amount) FROM tblinvoiceservicepaymentrecords WHERE invoiceid=tblservicepi.id) as total_paid';
            }
    ...
    {
        $data['prefix'] = get_option('service_pi_prefix');

        $data['number_format'] = get_option('service_pi_number_format');
        ...
        $this->db->insert('tblservicepi', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {

            // Update next invoice number in settings
            $this->db->where('name', 'next_service_pi_number');
            ...
            handle_tags_save($tags, $insert_id, 'service_pi');

            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'service_pi')) {
                    ...
                    _maybe_insert_post_item_tax($itemid, $item, $insert_id, 'service_pi');
                }
            }

            // update_sales_total_tax_column($insert_id, 'service_pi', 'tblservicepi');
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $number = format_service_pi_number($id);

//        do_action('before_invoice_deleted', $id);
        $this->db->where('id', $id);
        $this->db->delete('tblservicepi');
        if ($this->db->affected_rows() > 0) {
            ...
            delete_tracked_emails($id, 'service_pi');
            ...
            $this->db->where('relid IN (SELECT id from tblitems_in WHERE rel_type="service_invoice" AND rel_id="' . $id . '")');
            $this->db->where('fieldto', 'items');
            $this->db->delete('tblcustomfieldsvalues');

            $items = get_items_by_type('service_pi', $id);
            ...
            $this->db->delete('tblinvoiceservicepaymentrecords');
            ...
            $this->db->where('fieldto', 'service_pi');
            $this->db->delete('tblcustomfieldsvalues');
            ...
            if ($simpleDelete == false) {
                logActivity('Service Pi Deleted [' . $number . ']');
            }
    ...
    public function delete_attachment($id)
    {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink(get_upload_path_by_type('service_pi') . $attachment->rel_id . '/' . $attachment->file_name);
            }
            ...
            if (is_dir(get_upload_path_by_type('service_pi') . $attachment->rel_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files(get_upload_path_by_type('service_pi') . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    // okey only index.html so we can delete the folder also
                    delete_dir(get_upload_path_by_type('service_pi') . $attachment->rel_id);
                }
            }
        }
    ...
    public function mark_as_cancelled($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->log_invoice_activity($id, 'marked service pi as cancelled');
    ...
    public function send_invoice_to_client($id, $template = '', $attachpdf = true, $cc = '', $manually = false)
    {
        ...
        $this->emails_model->set_rel_type('service_pi');
                ...
                if ($contact_id != '') {
                    ...
                    if ($this->input->post('email_attachments')) {
                        ...
                        foreach ($_other_attachments as $attachment) {
                            ...
                            $this->emails_model->add_attachment([
                                'attachment' => get_upload_path_by_type('service_pi') . $id . '/' . $_attachment->file_name,
    ...
    public function check_for_merge_invoice($client_id, $current_invoice = '')
    {
        if ($current_invoice != '') {
            ...
            $row = $this->db->get('tblservicepi')->row();
        ...
        if (!$has_permission_view) {
            $whereUser = get_service_pis_where_sql_for_staff(get_staff_user_id());
            $this->db->where('(' . $whereUser . ')');
        }
        if ($current_invoice != '') {
            $this->db->where('id !=', $current_invoice);
    ...
    public function copy($id)
    {
        ...
        if ($id) {
            ...
            $custom_fields = get_custom_fields('service_pi');
            foreach ($custom_fields as $field) {
                $value = get_custom_field_value($_invoice->id, $field['id'], 'service_pi', false);
                if ($value == '') {
                    continue;
                }
            ...
            $tags = get_tags_in($_invoice->id, 'service_pi');
            handle_tags_save($tags, $id, 'service_pi');
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['tags'])) {
            if (handle_tags_save($data['tags'], $id, 'service_pi')) {
                $affectedRows++;
            }
        }
        ...
        foreach ($data['removed_items'] as $remove_item_id) {
            ...
            if (handle_removed_sales_item_post($remove_item_id, 'service_pi')) {
                $affectedRows++;
        ...
        $data['total_tax'] = ($data['total'] + $data['discount_total']) - ($data['subtotal'] + $data['packing_and_forwarding'] + $data['servicecharge']);
        $this->db->where('id', $id);
        $this->db->update('tblservicepi', $data);
        ...
        if (count($items) > 0) {
            foreach ($items as $key => $item) {
                ...
                if (!isset($item['taxname']) || (isset($item['taxname']) && count($item['taxname']) == 0)) {
                    if (delete_taxes_from_item($item['itemid'], 'service_pi')) {
                        $affectedRows++;
                    }
                } else {
                    ...
                    if (_maybe_insert_post_item_tax($item['itemid'], $item, $id, 'service_pi')) {
                        $affectedRows++;
                    }
                }
            }
        ...
        foreach ($newitems as $key => $item) {
            if ($new_item_added = add_new_sales_item_post($item, $id, 'service_pi')) {
                if (isset($billed_tasks[$key])) {
                    foreach ($billed_tasks[$key] as $_task_id) {
                        $this->db->insert('tblitemsrelated', [
                ...
                _maybe_insert_post_item_tax($new_item_added, $item, $id, 'service_pi');
        ...
        if ($affectedRows > 0) {
            // update_sales_total_tax_column($id, 'service_pi', 'tblservicepi');
    ...
    public function convert_to_invoice($id, $client = false, $draft_invoice = false)
    {
        ...
        $new_invoice_data['number']     = get_option('next_service_invoice_number');
        $new_invoice_data['date']       = _d(date('Y-m-d'));
        ...
        $new_invoice_data['show_shipping_on_invoice'] = '';
        ...
        foreach ($_estimate->items as $item) {
            ...
            $new_invoice_data['newitems'][$key]['unit']             = $item['unit'];
            $new_invoice_data['newitems'][$key]['taxname']          = [];
            $taxes                                                  = get_service_pi_item_taxes($item['id']);
            foreach ($taxes as $tax) {
                // tax name is in format TAX1|10.00
                array_push($new_invoice_data['newitems'][$key]['taxname'], $tax['taxname']);
            }
        ...
        $id = $this->service_invoices_model->add($new_invoice_data);
        if ($id) {
            // Customer accepted the estimate and is auto converted to invoice
            if (!is_staff_logged_in()) {
                $this->db->where('rel_type', 'service_invoice');
                $this->db->where('rel_id', $id);
                $this->db->delete('tblsalesactivity');
                $this->invoices_model->log_invoice_activity($id, 'invoice_activity_auto_converted_from_estimate', true, serialize([
                    '<a href="' . admin_url('service_pi/list_invoices/' . $_estimate->id) . '">' . format_service_pi_number($_estimate->id) . '</a>',
                ]));
            }
            ...
            if (is_custom_fields_smart_transfer_enabled()) {
                ...
                foreach ($cfEstimates as $field) {
                    ...
                    if (isset($tmpSlug[1])) {
                        $this->db->where('fieldto', 'service_invoice');
                        $this->db->where('slug LIKE "service_invoice_' . $tmpSlug[1] . '%" AND type="' . $field['type'] . '" AND options="' . $field['options'] . '" AND active=1');
                        ...
                        if (count($cfTransfer) == 1 && ((similarity($field['name'], $cfTransfer[0]['name']) * 100) >= CUSTOM_FIELD_TRANSFER_SIMILARITY)) {
                            $value = get_custom_field_value($_estimate->id, $field['id'], 'service_pi', false);
            ...
            if ($client == false) {
                $this->log_invoice_activity($_estimate->id, 'estimate_activity_converted', false, serialize([
                    '<a href="' . admin_url('service_invoices/list_invoices/' . $id) . '">' . format_service_invoice_number($id) . '</a>',
                ]));
            }
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM tblservicepi ORDER BY year DESC')->result_array();
    }
}
