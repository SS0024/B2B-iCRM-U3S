...
class Invoice_payments_model extends CRM_Model
{
    ...
    {
        $this->db->select('*,tblinvoiceservicepaymentrecords.id as paymentid');
        $this->db->join('tblinvoicepaymentsmodes', 'tblinvoicepaymentsmodes.id = tblinvoiceservicepaymentrecords.paymentmode', 'left');
        $this->db->order_by('tblinvoiceservicepaymentrecords.id', 'asc');
        $this->db->where('invoiceid', $invoiceid);
        $payments = $this->db->get('tblinvoiceservicepaymentrecords')->result_array();
        // Since version 1.0.1
    ...
    public function process_payment($data, $invoiceid = '')
    {
        ...
        // Is online payment mode request by client or staff
            ...
            $invoice = $this->service_invoices_model->get($invoiceid);
            ...
            if (get_option('allow_payment_amount_to_be_modified') == 0) {
                $data['amount'] = get_service_invoice_total_left_to_pay($invoiceid, $invoice->total);
            }
    ...
    public function add($data, $subscription = false)
    {
        ...
        $this->db->insert('tblinvoiceservicepaymentrecords', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            $invoice      = $this->service_invoices_model->get($data['invoiceid']);
            $force_update = false;
...
//            update_service_invoice_status($data['invoiceid'], $force_update);
            ...
            $this->service_invoices_model->log_invoice_activity($data['invoiceid'], $activity_lang_key, !is_staff_logged_in() ? true : false, serialize([
                format_money($data['amount'], $invoice->symbol),
                '<a href="' . admin_url('service_payments/payment/' . $insert_id) . '" target="_blank">#' . $insert_id . '</a>',
            ]));

            logActivity('Payment Recorded [ID:' . $insert_id . ', Invoice Number: ' . format_service_invoice_number($invoice->id) . ', Total: ' . format_money($data['amount'], $invoice->symbol) . ']');

            // Send email to the client that the payment is recorded
            $payment               = $this->get($insert_id);
            $payment->invoice_data = $this->service_invoices_model->get($payment->invoiceid);
            ...
            if (!isset($do_not_send_email_template)
                ...
                if ($subscription != false) {
                    ...
                    if ($after_success == 'send_invoice_and_receipt' || $after_success == 'send_invoice') {
                        $invoice_number       = format_service_invoice_number($payment->invoiceid);
                ...
                if (count($emails_sent) > 0) {
                    ...
                    $this->service_invoices_model->log_invoice_activity($invoice->id, $activity_lang_key, false, $additional_activity_data);
                }
            }
                ...
                if (get_option('notification_when_customer_pay_invoice') == 1) {
                    ...
                    $notified = add_notification([
                        ...
                        'additional_data' => serialize([
                            format_service_invoice_number($invoice->id),
                        ]),
                    ]);
                    if ($notified) {
    ...
    public function get($id)
    {
        ...
        $this->db->join('tblinvoicepaymentsmodes', 'tblinvoicepaymentsmodes.id = tblinvoiceservicepaymentrecords.paymentmode', 'left');
        $this->db->order_by('tblinvoiceservicepaymentrecords.id', 'asc');
        $this->db->where('tblinvoiceservicepaymentrecords.id', $id);
        $payment = $this->db->get('tblinvoiceservicepaymentrecords')->row();
        if (!$payment) {
            return false;
        }
        // Since version 1.0.1
    ...
    public function update($data, $id)
    {
        ...
        $data = $_data['data'];
        $this->db->where('id', $id);
        $this->db->update('tblinvoiceservicepaymentrecords', $data);
        if ($this->db->affected_rows() > 0) {
            if ($data['amount'] != $payment->amount) {
//                update_service_invoice_status($payment->invoiceid);
            }
    ...
    {
        $current         = $this->get($id);
        $current_invoice = $this->service_invoices_model->get($current->invoiceid);
        ...
        if ($this->db->affected_rows() > 0) {
//            update_service_invoice_status($invoiceid);
            $this->service_invoices_model->log_invoice_activity($invoiceid, 'invoice_activity_payment_deleted', false, serialize([
                $current->paymentid,
                format_money($current->amount, $current_invoice->symbol),
            ]));
            logActivity('Payment Deleted [ID:' . $id . ', Invoice Number: ' . format_service_invoice_number($current->id) . ']');
