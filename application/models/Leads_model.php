...
class Leads_model extends CRM_Model
{
    ...
    public function add($data)
    {
        ...
        if (isset($data['month_of_lead_closer'])) {
            $data['month_of_lead_closer'] = to_sql_date($data['month_of_lead_closer']);
        }
        ...
        if (is_array($data['consumable'])){
            $data['consumable'] = implode(', ', $data['consumable']);
        }else{
            $data['consumable'] = '';
        }
        if(!isset($data['cfm_required'])){
            $data['cfm_required'] = 0;
        }
        if(!isset($data['pressure_required'])){
            $data['pressure_required'] = 0;
        }
        if(!isset($data['machine_required'])){
            $data['machine_required'] = 0;
        }
        if(!isset($data['buy_back_potential'])){
            $data['buy_back_potential'] = 0;
        }
        if(!isset($data['buy_back_proposal'])){
            $data['buy_back_proposal'] = 0;
        }
        if(!isset($data['existing_model_oil_type'])){
            $data['existing_model_oil_type'] = 0;
        }
    ...
    public function add_attachment_to_database($lead_id, $attachment, $external = false, $form_activity = false,$profile_type = '')
    {
		if(!empty($profile_type) && $profile_type == 'lead_profile'){
			$this->misc_model->add_attachment_to_database($lead_id, 'lead_profile', $attachment, $external);
		}else{
     ...
     * @return array
     */
    public function get_lead_profile_attachment($id = '')
    {
        
        $this->db->where('rel_id', $id);
        $this->db->where('rel_type', 'lead_profile');
       // $this->db->order_by('dateadded', 'DESC');
        return $this->db->get('tblfiles')->row();
    }
}
