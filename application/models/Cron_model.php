...
class Cron_model extends CRM_Model
{
    ...
    private function purchaseorder_overdue()
    {
        $invoice_auto_operations_hour = 24;
        if ($invoice_auto_operations_hour == '') {
            $invoice_auto_operations_hour = 9;
        }
        ...
        $this->db->select('id,sale_agent,expirydate');
        $this->db->from('tblpurchaseorder');
        $this->db->where('(expirydate > DATE_SUB("2017-06-15", INTERVAL 5 DAY))');
        $invoices = $this->db->get()->result_array();
        $now = time();
        foreach ($invoices as $invoice) {
            $data['date']        = date('Y-m-d H:i:s');
            $data['description'] = 'Purchase order supply date is near.';
            $data['rel_id'] = $invoice['id'];
            $data['rel_type'] = 'purchaseorder';
            $data['creator']     = $invoice['sale_agent'];
            $data['staff']     = $invoice['sale_agent'];
