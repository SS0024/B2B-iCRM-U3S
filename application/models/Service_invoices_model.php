...
class Service_invoices_model extends CRM_Model
{
    ...
    {
        return $this->db->query("SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, ' ', lastname) as full_name FROM tblserviceinvoices JOIN tblstaff ON tblstaff.staffid=tblserviceinvoices.sale_agent WHERE sale_agent != 0")->result_array();
    }
    ...
    public function get_invoice_recurring_invoices($id)
    {
        ...
        $invoices           = $this->db->get('tblserviceinvoices')->result_array();
        $recurring_invoices = [];
    ...
    {
        $this->db->select('*, tblcurrencies.id as currencyid, tblserviceinvoices.id as id, tblcurrencies.name as currency_name');
        $this->db->from('tblserviceinvoices');
        $this->db->join('tblcurrencies', 'tblcurrencies.id = tblserviceinvoices.currency', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where('tblserviceinvoices' . '.id', $id);
            $invoice = $this->db->get()->row();
            if ($invoice) {
                $invoice->total_left_to_pay = get_service_invoice_total_left_to_pay($invoice->id, $invoice->total);

                $invoice->items       = get_items_by_type('service_invoice', $id);
    ...
    public function get_invoices_total($data)
    {
        ...
        $has_permission_view                = has_permission('service_invoices', '', 'view');
        $has_permission_view_own            = has_permission('service_invoices', '', 'view_own');

        for ($i = 1; $i <= 3; $i++) {
            $select = 'id,total';
            if ($i == 1) {
                $select .= ', (SELECT total - (SELECT COALESCE(SUM(amount),0) FROM tblinvoiceservicepaymentrecords WHERE invoiceid = tblserviceinvoices.id) - (SELECT COALESCE(SUM(amount),0) FROM tblcredits WHERE tblcredits.invoice_id=tblserviceinvoices.id)) as outstanding';
            } elseif ($i == 2) {
                $select .= ',(SELECT SUM(amount) FROM tblinvoiceservicepaymentrecords WHERE invoiceid=tblserviceinvoices.id) as total_paid';
            }
            ...
            if (!$has_permission_view) {
                $whereUser = get_service_invoices_where_sql_for_staff(get_staff_user_id());
    ...
    {
        $data['prefix'] = get_option('service_invoice_prefix');

        $data['number_format'] = get_option('service_invoice_number_format');
		...
		$data['total_tax'] = ($data['total'] + $data['discount_total']) - ($data['subtotal']);
        $this->db->insert('tblserviceinvoices', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {

            // Update next invoice number in settings
            $this->db->where('name', 'next_service_invoice_number');
            ...
            handle_tags_save($tags, $insert_id, 'service_invoice');
            ...
            foreach ($invoices_to_merge as $m) {
                ...
                if ($cancel_merged_invoices == false) {
                    ...
                    if ($this->mark_as_cancelled($m)) {
                        ...
                        $note       = 'Merged into invoice ' . format_service_invoice_number($insert_id);
                        if ($admin_note != '') {
                            $admin_note .= "\n\r" . $note;
                        } else {
                            $admin_note = $note;
            ...
            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'service_invoice')) {
                    ...
                    _maybe_insert_post_item_tax($itemid, $item, $insert_id, 'service_invoice');
                }
            }

            // update_sales_total_tax_column($insert_id, 'service_invoice', 'tblserviceinvoices');
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $number = format_service_invoice_number($id);

        do_action('before_invoice_deleted', $id);
        $this->db->where('id', $id);
        $this->db->delete('tblserviceinvoices');
        if ($this->db->affected_rows() > 0) {
            ...
            delete_tracked_emails($id, 'service_invoice');
            ...
            $items = get_items_by_type('service_invoice', $id);
            ...
            if ($simpleDelete == false) {
                logActivity('Service invoice Deleted [' . $number . ']');
            }
    ...
    public function delete_attachment($id)
    {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink(get_upload_path_by_type('service_invoice') . $attachment->rel_id . '/' . $attachment->file_name);
            }
            ...
            if (is_dir(get_upload_path_by_type('service_invoice') . $attachment->rel_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files(get_upload_path_by_type('service_invoice') . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    // okey only index.html so we can delete the folder also
                    delete_dir(get_upload_path_by_type('service_invoice') . $attachment->rel_id);
                }
            }
        }
    ...
    public function send_invoice_to_client($id, $template = '', $attachpdf = true, $cc = '', $manually = false)
    {
        ...
        $this->emails_model->set_rel_type('service_invoice');
        ...
        if (is_array($sent_to) && count($sent_to) > 0) {
            ...
            if ($attachpdf) {
                ...
                $pdf          = service_invoice_pdf($_pdf_invoice);
                ...
                if ($contact_id != '') {
                    ...
                    if ($this->input->post('email_attachments')) {
                        ...
                        foreach ($_other_attachments as $attachment) {
                            ...
                            $this->emails_model->add_attachment([
                                'attachment' => get_upload_path_by_type('service_invoice') . $id . '/' . $_attachment->file_name,
    ...
    public function check_for_merge_invoice($client_id, $current_invoice = '')
    {
        if ($current_invoice != '') {
            ...
            $row = $this->db->get('tblserviceinvoices')->row();
    ...
    public function copy($id)
    {
        ...
        if ($id) {
            ...
            $custom_fields = get_custom_fields('service_invoice');
            foreach ($custom_fields as $field) {
                $value = get_custom_field_value($_invoice->id, $field['id'], 'service_invoice', false);
                if ($value == '') {
                    continue;
                }
            ...
            $tags = get_tags_in($_invoice->id, 'service_invoice');
            handle_tags_save($tags, $id, 'service_invoice');
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['tags'])) {
            if (handle_tags_save($data['tags'], $id, 'service_invoice')) {
                $affectedRows++;
            }
        }
        ...
        foreach ($data['removed_items'] as $remove_item_id) {
            ...
            if (handle_removed_sales_item_post($remove_item_id, 'service_invoice')) {
                $affectedRows++;
        ...
        $data['total_tax'] = ($data['total'] + $data['discount_total']) - ($data['subtotal']);
        $this->db->where('id', $id);
        $this->db->update('tblserviceinvoices', $data);
        ...
        if (count($items) > 0) {
            foreach ($items as $key => $item) {
                ...
                if (!isset($item['taxname']) || (isset($item['taxname']) && count($item['taxname']) == 0)) {
                    if (delete_taxes_from_item($item['itemid'], 'service_invoice')) {
                        $affectedRows++;
                    }
                } else {
                    ...
                    if (_maybe_insert_post_item_tax($item['itemid'], $item, $id, 'service_invoice')) {
                        $affectedRows++;
                    }
                }
            }
        ...
        foreach ($newitems as $key => $item) {
            if ($new_item_added = add_new_sales_item_post($item, $id, 'service_invoice')) {
                if (isset($billed_tasks[$key])) {
                    foreach ($billed_tasks[$key] as $_task_id) {
                        $this->db->insert('tblitemsrelated', [
                ...
                _maybe_insert_post_item_tax($new_item_added, $item, $id, 'service_invoice');
        ...
        if ($affectedRows > 0) {
            // update_sales_total_tax_column($id, 'service_invoice', 'tblserviceinvoices');
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM tblserviceinvoices ORDER BY year DESC')->result_array();
    }
}
