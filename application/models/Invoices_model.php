...
class Invoices_model extends CRM_Model
{
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            foreach ($items as $item) {
                add_hold_item(['description'=>$items['description'],'qty'=>$item['qty']],$id,'invoice');
            }
    ...
    public function add($data, $expense = false)
    {
        ...
        foreach ($items as $item){
            ...
            if ($stock < $item['qty']){
                set_alert('danger', _l('Check hold item stock'));
                return false;
            }
        }
...
//                plus_item_hold_quantity($item['description'],$item['qty']);
                add_hold_item(['description'=>$item['description'],'qty'=>$item['qty']],$insert_id,'invoice');
...
//                minus_item_hold_quantity($item->description,$item->qty);
                remove_hold_item(['description'=>$item->description], $id, 'invoice');
...
//            minus_item_hold_quantity($original_item->description,$original_item->qty);
            remove_hold_item(['description'=>$original_item->description], $id, 'invoice');
...
//                    plus_item_hold_quantity($original_item->description,$item['qty']);
                    add_hold_item(['description'=>$original_item->description,'qty'=>$item['qty']],$id,'invoice');
                }
...
//            plus_item_hold_quantity($item['description'],$item['qty']);
            add_hold_item(['description'=>$item['description'],'qty'=>$item['qty']],$id,'invoice');
...
//                minus_item_hold_quantity($data['description'],$data['quantity']);
                remove_hold_item(['description'=>$item->description],$data['invoice_id'], 'invoice');
                $this->invoice_items_model->syncProductQty($item->id, $data['warehouse_id']);
                $this->purchases_model->addStockTransaction(
                    [ 'rel_type'=>'sale', 'id' => $data['item_id'], 'quantity' => $data['quantity'] ]
                );
            }
        }
    }
