...
//    private $shipping_fields = ['shipping_street', 'shipping_city', 'shipping_city', 'shipping_state', 'shipping_zip', 'shipping_country'];
    ...
    {
        $this->db->select('*, tblpurchases.id as id, tblsuppliers.company as supplier, tblsuppliers.address as supplier_address, tblsuppliers.city as supplier_city, tblsuppliers.zip as supplier_zip, tblsuppliers.state as supplier_state, tblsuppliers.gst_number as supplier_gst_number');
        $this->db->from('tblpurchases');
        $this->db->join('tblsuppliers', 'tblsuppliers.id = tblpurchases.supplier_id', 'left');
        $this->db->join('tblwarehouses', 'tblwarehouses.id = tblpurchases.warehouse_id', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
...
//        $data = $this->map_shipping_columns($data, $expense);
        ...
        if ($data['supply_date']) {
            $data['supply_date'] = to_sql_date($data['supply_date']);
        }
        ...
        $data['total_tax'] = ($data['total']) - ($data['subtotal'] + $data['packing_and_forwarding'] + $data['servicecharge']);
        $this->db->insert('tblpurchases', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {

            // Update next invoice number in settings
            $this->db->where('name', 'next_purchase_number');
...
//            update_invoice_status($insert_id);
            ...
            foreach ($items as $key => $item) {
                ...
                $item['date'] = $data['date'];
                $this->add_purchase_item($insert_id, $item);
            }
            if ($data['status'] == 'full_received' || $data['status'] == 'partial_received') {
                $this->load->model('invoice_items_model');
                $this->invoice_items_model->syncQuantity(NULL, $insert_id);
            }
            $this->log_invoice_activity($insert_id, 'created the purchase');
            return $insert_id;
        }
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['removed_items'])) {
            foreach ($data['removed_items'] as $remove_item_id) {
                ...
                $this->db->delete('tblpurchaseitems');
                if ($this->db->affected_rows() > 0) {
        ...
        foreach ($newitems as $key => $item) {
            if ($item['qty'] == $item['quantity_received']) {
                $itemWiseStatus[] = 'full_received';
            } elseif ($item['quantity_received'] == 0) {
                $itemWiseStatus[] = 'ordered';
            } elseif ($item['qty'] > $item['quantity_received']) {
                $itemWiseStatus[] = 'partial_received';
            }
        }
        foreach ($items as $key => $item) {
            if ($item['qty'] == $item['quantity_received']) {
        ...
        if (in_array('full_received', $itemWiseStatus) && !in_array('partial_received', $itemWiseStatus) && !in_array('ordered', $itemWiseStatus)) {
            $data['status'] = 'full_received';
        } elseif (in_array('partial_received', $itemWiseStatus)) {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($original_number != $data['number']) {
                $this->log_invoice_activity($original_invoice->id, "Purchase number changed from {$original_number_formatted} to " . format_purchase_number($original_invoice->id), false, serialize([
        ...
        if (count($items) > 0) {
            foreach ($items as $key => $item) {
                ...
                if ($original_item->quantity_received != $item['quantity_received']) {
                    $this->db->where('id', $id);
                ...
                $updateDataItem = array(
                    ...
                    'unit_cost' => $data['item_amount'],
                    'item_discount' => $data['item_discount'],
                    'real_unit_cost' => $item['rate'],
                    'quantity' => $item['qty'],
                    'product_unit_id' => $item['unit'] ? $item['unit'] : 0,
                    'product_unit_code' => $item['unit'] ? $item['unit'] : '',
                    'unit_quantity' => $item['qty'],
//                    'quantity_balance' => $data['status'] == 'received' || $data['status'] == 'partial_received' ? $item['quantity_received'] : 0,
                    'quantity_received' => $data['status'] == 'full_received' || $data['status'] == 'partial_received' ? $item['quantity_received'] : 0,
...
//                    'tax' => $item['taxname'],
                ...
                if ($original_item->quantity_balance != $updateDataItem['quantity_balance']) {
                    $this->log_invoice_activity($id, "updated item quantity balance from {$original_item->quantity_balance} to " . $updateDataItem['quantity_balance'], false, serialize([
                        $original_item->quantity_balance,
                        $updateDataItem['quantity_balance'],
                    ]));
                    $affectedRows++;
                }
                if ($original_item->quantity_received != $updateDataItem['quantity_received']) {
                    $this->log_invoice_activity($id, "updated item quantity received from {$original_item->quantity_received} to " . $updateDataItem['quantity_received'], false, serialize([
                        $original_item->quantity_received,
                        $updateDataItem['quantity_received'],
                    ]));
                    $affectedRows++;
                }
                if ($original_item->quantity != $updateDataItem['quantity']) {
                    $this->log_invoice_activity($id, "updated item quantity from {$original_item->quantity} to " . $updateDataItem['quantity'], false, serialize([
...
//                    $item_taxes        = get_invoice_item_taxes($item['itemid']);
        ...
        foreach ($newitems as $key => $item) {
            ...
            $this->add_purchase_item($id, $item);
            $affectedRows++;
        }
...
//            do_action('after_invoice_updated', $id);
    ...
    public function add_purchase_item($purchase_id, $data)
    {
        $this->load->model('invoice_items_model');
        ...
        $item = array(
            ...
            'quantity' => $data['qty'],
            'product_unit_id' => $data['unit'] ? $data['unit'] : 0,
            'product_unit_code' => $data['unit'] ? $data['unit'] : '',
            'unit_quantity' => $data['qty'],
            'quantity_balance' => 0,
            'quantity_received' => $data['status'] == 'full_received' || $data['status'] == 'partial_received' ? $data['quantity_received'] : 0,
            'warehouse_id' => $data['warehouse_id'],
            'date' => date('Y-m-d', strtotime($data['date'])),
            'status' => $data['status'],
//            'tax' => $data['taxname'],
        ...
        $this->db->insert('tblpurchaseitems', $item);
        $itemId = $this->db->insert_id();
        foreach ($data['taxname'] as $taxname) {
            if ($taxname != '') {
                $tax_array = explode('|', $taxname);
                if (isset($tax_array[0]) && isset($tax_array[1])) {
                    $tax_name = trim($tax_array[0]);
        ...
        $this->log_invoice_activity($purchase_id, 'invoice_estimate_activity_added_item', false, serialize([
            $item['product_code'],
        ]));
    }
