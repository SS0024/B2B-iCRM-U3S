...
//        $this->db->where('staffid', get_staff_user_id());
        $this->db->group_start();
        $this->db->where('staffid', get_staff_user_id());
        $this->db->or_where('addedby', get_staff_user_id());
        $this->db->group_end();
        if (is_numeric($id)) {
    ...
    {
        $this->db->select('tbltodoitems.*, CONCAT(assigned.firstname, \' \', assigned.lastname) as assigned_name, CONCAT(added.firstname, \' \', added.lastname) as added_name');
        $this->db->from('tbltodoitems');
        $this->db->join('tblstaff as assigned', 'assigned.staffid = tbltodoitems.staffid', 'left');
        $this->db->join('tblstaff as added', 'added.staffid = tbltodoitems.addedby', 'left');
        $this->db->where('tbltodoitems.finished', $finished);
        $this->db->group_start();
        $this->db->where('tbltodoitems.staffid', get_staff_user_id());
        $this->db->or_where('tbltodoitems.addedby', get_staff_user_id());
        $this->db->group_end();
        $this->db->order_by('tbltodoitems.item_order', 'asc');
