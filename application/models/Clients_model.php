...
class Clients_model extends CRM_Model
{
    ...
    public function searchDivisions($q, $userId)
    {
        $this->db->select('id, division as name');
        $this->db->like('division', $q);
        $this->db->where('userid', $userId);

        $items = $this->db->get('tbldivision')->result_array();
    ...
    public function add($data, $client_or_lead_convert_request = false)
    {
        ...
        if ($userid) {
            ...
            if ($client_or_lead_convert_request == true) {
                $contact_data['division'] = $divsionId;
    ...
    public function update($data, $id, $client_request = false)
    {
        ...
        if (isset($data['show_primary_contact']) && !$data['show_primary_contact']) {
            unset($data['show_primary_contact']);
        }
     ...
     * @return  array
     */
    public function get_customer_billing_and_shipping_division_details($id)
    {
        $this->db->select('address as billing_street,city as billing_city,state as billing_state,pincode as billing_zip,102 as billing_country,address as shipping_street,city as shipping_city,state as shipping_state,pincode as shipping_zip,102 as shipping_country');
    ...
    */
    ...
    public function get_task_customers(){
        $this->db->select('
        (CASE 
        WHEN rel_type="contract" THEN (SELECT client  FROM tblcontracts WHERE tblcontracts.id = tblstafftasks.rel_id LIMIT 1)
        WHEN rel_type="customer" THEN tblstafftasks.rel_id
        ELSE NULL
        END) as id,
        (CASE 
        WHEN rel_type="contract" THEN (SELECT '.get_sql_select_client_company().' FROM tblcontracts LEFT JOIN tblclients ON tblclients.userid = tblcontracts.client WHERE tblcontracts.id = tblstafftasks.rel_id LIMIT 1)
        WHEN rel_type="customer" THEN (SELECT '.get_sql_select_client_company().' FROM tblclients where tblclients.userid = tblstafftasks.rel_id LIMIT 1)
        ELSE NULL
        END) as name');
        $items = $this->db->get('tblstafftasks')->result_array();
        return array_map("unserialize", array_unique(array_map("serialize", $items)));;
    }

    public function get_divisions($id = null)
    {
        if(isset($id)){
            $this->db->where('userid', $id);
        }
        return $this->db->get('tbldivision')->result();
    }

    public function add_division($data, $customer_id, $not_manual_request = false)
    {

        $this->db->insert('tbldivision', $data);
        $contact_id = $this->db->insert_id();

        logActivity('Division Created [ID: ' . $contact_id . ']');
        do_action('contact_created', $contact_id);
    ...
    public function update_division($data, $id, $client_request = false)
    {
        $affectedRows = 0;
        $this->db->where('id',$id);
        $this->db->update('tbldivision', $data);
        $contact_id = $this->db->insert_id();

        logActivity('Division updated [ID: ' . $contact_id . ']');
        do_action('contact_created', $contact_id);
