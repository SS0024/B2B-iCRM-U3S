...
class Inquiries_model extends CRM_Model
{
    private $statuses;
    ...
    {
        return $this->db->query('SELECT DISTINCT(assigned) as sale_agent FROM tblinquiries WHERE assigned != 0')->result_array();
    }

    public function get_proposals_years()
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM tblinquiries')->result_array();
    }
    ...
    public function do_kanban_query($status, $search = '', $page = 1, $sort = [], $count = false)
    {
        ...
        if ($search != '') {
            if (!_startsWith($search, '#')) {
                ...
                $this->db->where('tblinquiries.id IN
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['follow_up_date'])){
            $data['follow_up_date'] = to_sql_date($data['follow_up_date']);
        }
        $items = [];
        if (isset($data['items'])) {
            $items = $data['items'];
        ...
        if (isset($data['tags'])) {
            if (handle_tags_save($data['tags'], $id, 'inquiry')) {
                $affectedRows++;
            }
        }
        ...
        foreach ($data['removed_items'] as $remove_item_id) {
            ...
            if (handle_removed_sales_item_post($remove_item_id, 'inquiry')) {
                $affectedRows++;
        ...
        if (!empty($div_cons)) {
            handle_removed_div_cons_post($id, 'inquiry');
            foreach ($div_cons as $con => $div) {
                if (isset($div)) {
                    add_new_div_con_item_post($div, $con, $id, 'inquiry');
                }
            }
        }
        ...
        $this->db->where('id', $id);
        $this->db->update('tblinquiries', $data);
        if ($this->db->affected_rows() > 0) {
            // Check for status change
            if ($original_status != $data['status']) {
                $this->log_estimate_activity($current_proposal->id, 'Inquiry Status Updated: From: <original_status>' . $original_status . '</original_status> to <new_status>' . $data['status'] . '</new_status>', false);
            }
            if ($original_number != $data['number']) {
                $this->log_estimate_activity($current_proposal->id, 'Inquiry number changed from '.$original_number.' to '.format_inquiry_number($current_proposal->id), false);
            }
            $affectedRows++;
        ...
        foreach ($items as $key => $item) {
            ...
            if (!isset($item['taxname']) || (isset($item['taxname']) && count($item['taxname']) == 0)) {
                if (delete_taxes_from_item($item['itemid'], 'inquiry')) {
                    $affectedRows++;
                }
            } else {
                ...
                if (_maybe_insert_post_item_tax($item['itemid'], $item, $id, 'inquiry')) {
                    $affectedRows++;
                }
            }
        }
        ...
        foreach ($newitems as $key => $item) {
            ...
            if ($new_item_added = add_new_sales_item_post($item, $id, 'inquiry')) {
                _maybe_insert_post_item_tax($new_item_added, $item, $id, 'inquiry');
        ...
        $this->db->update('tblinquiries', $data);
        if ($affectedRows > 0) {
            // update_sales_total_tax_column($id, 'inquiry', 'tblinquiries');
            logActivity('Inquiry Updated [ID:' . $id . ']');
            $this->log_estimate_activity($id, 'Inquiry Updated [ID:' . $id . ']');
        }
    ...
    public function get($id = '', $where = [], $for_editor = false)
    {
        ...
        $this->db->select('*,tblcurrencies.id as currencyid, tblinquiries.id as id, tblcurrencies.name as currency_name, (CASE 
        WHEN rel_type="lead" THEN (SELECT CASE company WHEN "" THEN name ELSE company END as company FROM tblleads WHERE tblleads.id = tblinquiries.rel_id LIMIT 1)
        WHEN rel_type="customer" THEN (SELECT ' . get_sql_select_client_company() . ' FROM tblclients where tblclients.userid = tblinquiries.rel_id LIMIT 1)
        ELSE NULL
        END) as proposal_to, (CASE 
        WHEN rel_type="lead" THEN (SELECT tblleads.client_id FROM tblleads WHERE tblleads.id = tblinquiries.rel_id LIMIT 1)
        ELSE 0
        END) as lead_customer_id');
        $this->db->from('tblinquiries');
        $this->db->join('tblcurrencies', 'tblcurrencies.id = tblinquiries.currency', 'left');

        if (is_numeric($id)) {
            $this->db->where('tblinquiries.id', $id);
            $proposal = $this->db->get()->row();
            if ($proposal) {
                $proposal->attachments = $this->get_attachments($id);
                $proposal->purchaseorders = $this->get_purchaseorders($id);
                $proposal->items = get_items_by_type('inquiry', $id);
    ...
    public function get_purchaseorders($estimate_id, $id = '')
    {
        // If is passed id get return only 1 attachment
        if (is_numeric($id)) {
            $this->db->where('id', $id);
        } else {
            $this->db->where('inquiry_id', $estimate_id);
        }
        $result = $this->db->get('tblpurchaseorder');
        if (is_numeric($id)) {
            return $result->row();
        }
    ...
    public function get_attachments($proposal_id, $id = '')
    {
        ...
        $this->db->where('rel_type', 'inquiry');
    ...
    public function send_proposal_to_email($id, $template = '', $attachpdf = true, $cc = '')
    {
        ...
        $this->emails_model->set_rel_type('inquiry');
        ...
        if ($proposal->status == 6) {
            $this->db->where('id', $id);
            $this->db->update('tblinquiries', ['status' => 4]);
            $proposal = $this->get($id);
        }
        ...
        if ($this->input->post('email_attachments')) {
            ...
            foreach ($_other_attachments as $attachment) {
                ...
                $this->emails_model->add_attachment([
                    'attachment' => get_upload_path_by_type('inquiry') . $id . '/' . $_attachment->file_name,
    ...
    public function mark_action_status($status, $id, $client = false)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->log_estimate_activity($id, 'Inquiry marked as <status>' . format_proposal_status($status, '', false) . '</status>', false);
            logActivity('Inquiry Status Changes [InquiryID:' . $id . ', Status:' . format_proposal_status($status, '', false) . ',Client Action: ' . (int)$client . ']');
    ...
    public function copy($id)
    {
        ...
        $fields = $this->db->list_fields('tblinquiries');
        $insert_data = [];
        ...
        if ($id) {
            ...
            foreach ($custom_fields as $field) {
                $value = get_custom_field_value($proposal->id, $field['id'], 'inquiry', false);
                if ($value == '') {
                    continue;
                }
            ...
            handle_tags_save($tags, $id, 'inquiry');
    ...
    public function add($data)
    {
        ...
        $data['modifieddate'] = date('Y-m-d H:i:s');
        $data['total_tax'] = ($data['total'] + $data['discount_total']) - ($data['subtotal'] + $data['packing_and_forwarding'] + $data['servicecharge']);
        $this->db->insert('tblinquiries', $data);
        ...
        if ($insert_id) {
            ...
            handle_tags_save($tags, $insert_id, 'inquiry');

            foreach ($div_cons as $con => $div) {
                if (isset($div)) {
                    add_new_div_con_item_post($div, $con, $insert_id, 'inquiry');
                }
            }
            $isRfr = 0;
            foreach ($items as $key => $item) {
                ...
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'inquiry')) {
                    _maybe_insert_post_item_tax($itemid, $item, $insert_id, 'inquiry');
                }
            }
            $data['is_rfr'] = $isRfr;
            ...
            // update_sales_total_tax_column($insert_id, 'inquiry', 'tblinquiries');
            logActivity('New inquiry Created [ID:' . $insert_id . ']');
            $this->log_estimate_activity($insert_id, 'New inquiry created.');
    ...
    public function delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblinquiries');
        if ($this->db->affected_rows() > 0) {
            delete_tracked_emails($id, 'inquiry');
            handle_removed_div_cons_post($id, 'inquiry');
            // Get related tasks
            $this->db->where('rel_type', 'inquiry');
            ...
            logActivity('Inquiry Deleted [InquiryID:' . $id . ']');
    ...
    public function clear_signature($id)
    {
        ...
        $this->db->where('id', $id);
        $proposal = $this->db->get('tblinquiries')->row();

        if ($proposal) {
            $this->db->where('id', $id);
            $this->db->update('tblinquiries', ['signature' => null]);

            if (!empty($proposal->signature)) {
                unlink(get_upload_path_by_type('inquiry') . $id . '/' . $proposal->signature);
            }
    ...
    public function delete_attachment($id)
    {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink(get_upload_path_by_type('inquiry') . $attachment->rel_id . '/' . $attachment->file_name);
            }
            $this->db->where('id', $attachment->id);
            $this->db->delete('tblfiles');
            if ($this->db->affected_rows() > 0) {
                $deleted = true;
                logActivity('Inquiry Attachment Deleted [ID: ' . $attachment->rel_id . ']');
            }
            if (is_dir(get_upload_path_by_type('inquiry') . $attachment->rel_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files(get_upload_path_by_type('inquiry') . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    // okey only index.html so we can delete the folder also
                    delete_dir(get_upload_path_by_type('inquiry') . $attachment->rel_id);
                }
            }
        }
