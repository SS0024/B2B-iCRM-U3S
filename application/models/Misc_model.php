...
class Misc_model extends CRM_Model
{
    ...
    public function get_taxes_dropdown_template($name, $taxname, $type = '', $item_id = '', $is_edit = false, $manual = false)
    {
        ...
        $select = '<select class="selectpicker display-block tax" required data-width="100%" name="' . $name . '" multiple data-none-selected-text="' . _l('no_tax') . '">';
    ...
    public function get_suppliers_dropdown_template($item_key, $supplierid, $type = '', $item_id = '', $is_edit = false, $manual = false)
    {
        // if passed manually - like in proposal convert items or project
        if ($manual == true) {
            // + is no longer used and is here for backward compatibilities
            if (is_array($supplierid) || strpos($supplierid, '+') !== false) {
                if (!is_array($supplierid)) {
                    $__supplier = explode('+', $supplierid);
                } else {
                    $__supplier = $supplierid;
                }
                // Multiple taxes found // possible option from default settings when invoicing project
                $supplierid = [];
                foreach ($__supplier as $s) {
                    $supplier_array =  $s ;
                }
            } else {
                $supplier_array = explode($supplierid);
                // isset tax rate
                if (isset($supplier_array)) {
                    $CI = &get_instance();
        ...
        foreach ($Suppliers as $Supplier) {
            unset($Suppliers[$i]['id']);
            $Suppliers[$i]['id'] = $Supplier['id'];
            $i++;
        }
        if ($is_edit == true) {
            ...
            // Lets check the items taxes in case of changes.
            // Separate functions exists to get item taxes for Invoice, Estimate, Proposal, Credit Note
            $func_supplier = 'get_' . $type . '_item_taxes';
            if (function_exists($func_supplier)) {
                $item_suppliers = call_user_func($func_supplier, $item_id);
            }
            foreach ($item_suppliers as $item_supplier) {
                $new_supplier            = [];
                $new_supplier['id']    = $item_supplier['supplierid'];
        ...
        if (is_array($supplierid)) {
            foreach ($supplierid as $supplier) {
                ...
                // Check if really the taxname NAME|RATE don't exists in all taxes
                if (!value_exists_in_array_by_key($suppliers, 'name', $supplier)) {
                    if (!is_array($supplier)) {
                        $tmp_supplier = $supplier;
                    } else {
                        $tmp_supplier = $supplier['id'];
                        if ($tmp_supplier == '') {
                            continue;
                        }
                    }
                    $suppliers[] = ['id' => $tmp_supplier];
                }
            }
        }

        // Clear the duplicates
        $suppliers = array_map('unserialize', array_unique(array_map('serialize', $suppliers)));

        $select = '<select class="selectpicker display-block suppliers" required data-width="100%" name="newitems['.$item_key.'][suppliers][]" multiple data-none-selected-text="' . _l('Suppliers') . '">';
        ...
        foreach ($Spplier_list as $selected_supplier) {
            ...
            foreach ($suppliers as $supplier) {
                ...
                if ($selected_supplier['id'] == $supplier["id"]) {
                    $selected = "selected";
                }
            ...
            $select .= '<option value="' . $selected_supplier['id'] . '" ' . $selected . ' >' . $selected_supplier['name'] . '</option>';
        }
        $select .= '</select>';
        return $select;
    }

    public function get_item_group_dropdown_for_mfitems($item_key, $group_id){

        $this->db->order_by('name', 'asc');

        $itemGroups = $this->db->get('tblitems_groups')->result_array();
        return render_select('newitems['.$item_key.'][group_id]',$itemGroups,array('id','name'),'',$group_id);
    }

    public function get_item_group_dropdown_template(){
        $this->db->order_by('name', 'asc');

        $itemGroups = $this->db->get('tblitems_groups')->result_array();
        return render_select('group_id',$itemGroups,array('id','name'),'',$group_id);
    }
    ...
    public function _search_inquiries($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'inquiries',
            'search_heading' => _l('inquiries'),
        ];
        ...
        if ($has_permission_view_proposals || $has_permission_view_proposals_own || get_option('allow_staff_view_proposals_assigned') == '1') {
            if (is_numeric($q)) {
                $q = trim($q);
                $q = ltrim($q, '0');
            } elseif (_startsWith($q, get_option('inquiry_number_prefix'))) {
                $q = strafter($q, get_option('inquiry_number_prefix'));
                $q = trim($q);
                $q = ltrim($q, '0');
            }

            // Proposals
            $this->db->select('*,tblinquiries.id as id');
            $this->db->from('tblinquiries');
            $this->db->join('tblcurrencies', 'tblcurrencies.id = tblinquiries.currency');
            ...
            $this->db->where('(
                tblinquiries.id LIKE "' . $q . '%"
                OR tblinquiries.subject LIKE "%' . $q . '%"
                OR tblinquiries.content LIKE "%' . $q . '%"
                OR tblinquiries.proposal_to LIKE "%' . $q . '%"
                OR tblinquiries.zip LIKE "%' . $q . '%"
                OR tblinquiries.state LIKE "%' . $q . '%"
                OR tblinquiries.city LIKE "%' . $q . '%"
                OR tblinquiries.address LIKE "%' . $q . '%"
                OR tblinquiries.email LIKE "%' . $q . '%"
                OR tblinquiries.phone LIKE "%' . $q . '%"
                )');

            $this->db->order_by('tblinquiries.id', 'desc');
            if ($limit != 0) {
                $this->db->limit($limit);
            }
            $result['result'] = $this->db->get()->result_array();
    ...
    public function _search_tenders($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'tenders',
            'search_heading' => _l('Tenders'),
        ];
        ...
        if ($has_permission_view_proposals || $has_permission_view_proposals_own || get_option('allow_staff_view_proposals_assigned') == '1') {
            ...
            // Proposals
            $this->db->select('*,tbltenders.id as id');
            $this->db->from('tbltenders');
            $this->db->join('tblcurrencies', 'tblcurrencies.id = tbltenders.currency');
            ...
            $this->db->where('(
                tbltenders.id LIKE "' . $q . '%"
                OR tbltenders.subject LIKE "%' . $q . '%"
                OR tbltenders.content LIKE "%' . $q . '%"
                OR tbltenders.proposal_to LIKE "%' . $q . '%"
                OR tbltenders.zip LIKE "%' . $q . '%"
                OR tbltenders.state LIKE "%' . $q . '%"
                OR tbltenders.city LIKE "%' . $q . '%"
                OR tbltenders.address LIKE "%' . $q . '%"
                OR tbltenders.email LIKE "%' . $q . '%"
                OR tbltenders.phone LIKE "%' . $q . '%"
                )');

            $this->db->order_by('tbltenders.id', 'desc');
            if ($limit != 0) {
                $this->db->limit($limit);
            }
            $result['result'] = $this->db->get()->result_array();
    ...
    public function _search_contracts($q, $limit = 0)
    {
	 ...
	 public function add_attachment_to_temp_database($rel_type, $attachment, $external = false)
    {
        $data['dateadded'] = date('Y-m-d H:i:s');
        //$data['rel_id']    = $rel_id;
        if (!isset($attachment[0]['staffid'])) {
        ...
        $this->db->insert('tblfilestemp', $data);
        $insert_id = $this->db->insert_id();
    ...
    {
        $attachment = $this->get_temp_attachment($id);
        $deleted    = false;
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink(get_upload_path_by_type('invoice_payment_temp'). $attachment->file_name);
            }
            $this->db->where('id', $attachment->id);
            $this->db->delete('tblfilestemp');
            if ($this->db->affected_rows() > 0) {
                $deleted = true;
            }
        }
	 ...
	 public function get_temp_attachment($id = 0)
    {
       $this->db->where('id', $id);

       return $this->db->get('tblfilestemp')->row();
    }
}
